...
#### Correcciones de mantenimiento

- **CLA-529 (Done, 2026-09-01) — reloj del "due check" de A/B winner y follow-up.** `ExecuteCampaignJob` escribe `ab_test_started_at`/`finished_at` con `now()` de la app (`config('app.timezone')`, `Europe/Brussels`), pero `SelectAbWinnerCommand` y `DispatchFollowUpsCommand` filtraban con `DATE_SUB(NOW(), INTERVAL col HOUR)` usando el `NOW()` de MySQL — que sigue la tz de la **sesión** de la conexión, la cual puede diferir de `app.timezone`. Con la sesión en UTC (`@@session.time_zone = SYSTEM`, `@@system_time_zone = UTC`) el umbral se desfasa por el offset (2 h en verano): las campañas vencidas no entran al filtro y el `ExecuteCampaignJob` no se despacha, ni se marca `ab_winner_variant`/`followup_dispatched_at`. **Fix:** los 2 comandos capturan un único `$now = now()` y comparan con `DATE_SUB(?, …)` bindeando ese instante (reloj de la app, consistente con lo persistido). **`scheduled_at` NO estaba afectado** (`CampaignForm` lo convierte Brussels→UTC al guardar y `DispatchScheduledCampaignsCommand` compara con `now()->utc()`). **Riesgo DST residual (no resuelto):** en un cruce exacto de DST, un delay de "N horas" puede abarcar N-1 o N+1 horas reales; corregir duración absoluta exige normalizar almacenamiento y conexiones a UTC + backfill, alcance fuera de CLA-529. Verificado con suite global desde `migrate:fresh` en MySQL 8.4 aislado: `Tests: 1324, Failures: 0, Errors: 0, Skipped: 2` → 1322 passed (antes del fix: 11 fallos). Cobertura hermética nueva (`AbTestingTest`/`FollowUpTest` con `Carbon` fijado + sesión MySQL a `+00:00` + casos verano/invierno + restauración en `tearDown`; `CampaignFormScheduledAtTimezoneTest` verifica el valor crudo UTC vía `DB::table(...)`). **CI confirmada** (rama `release/laravel-13-rc1`, PR #9): run `33512861220` (head `4d0e32c`) y run de cierre `33526166018` (head `640242c`), 2026-09-01 — ambas `PHPUnit (PHP 8.4)` success, `2 skipped, 1322 passed (4285 assertions)`, 0 fallos / 0 errores; primera corrida de CI de la cadena Laravel 13 completamente verde. Commits `a735ee9` (fix) + `4d0e32c`/`640242c` (doc) en `origin` e integrados en `release/laravel-13-rc1` por fast-forward (head certificado por la CI de cierre: `640242c`). **CLA-529 cerrado en Linear como Done** (`completedAt` 2026-09-01T19:27:12Z, comentario `cf29afc6`). **Sin merge a `main`, staging ni deploy** — antes de desplegar: verificar `@@session.time_zone`/`@@system_time_zone`/`NOW()`/`UTC_TIMESTAMP()` en staging y producción, e inventariar A/B winners y follow-ups ya vencidos y no reclamados (entrarán en el siguiente tick tras el fix).
