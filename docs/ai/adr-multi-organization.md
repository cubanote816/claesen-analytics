...
Se registra con `$this->app->scoped(...)`. El worker de colas reinicia las instancias `scoped` entre jobs — **verificado directamente contra el vendor instalado en CLA-460 (cont.), no solo asumido**: `QueueServiceProvider.php:263` arma el callback `resetScope` (`flushSharedContext`/`withoutContext` del logger, `resetTotalQueryDuration` por conexión, `Container::forgetScopedInstances()`, `Facade::clearResolvedInstances()`) y lo pasa al `Worker`; `Worker.php:246-247`, dentro de `daemon()`, lo invoca después de cada job procesado. Con esto confirmado, un job-level middleware/`Queue::after` propio sería redundante como defensa contra fuga entre jobs — el framework ya la garantiza. **Se difiere deliberadamente a cuando exista un consumidor real** (mismo criterio que ya fijaba el docblock de `OrganizationContext` desde P2): hoy ningún `Job` del repo referencia esta clase (`grep -rln OrganizationContext Modules/*/Jobs` vacío, confirmado en CLA-460 cont.), así que construir esa capa ahora sería scaffolding sin consumidor. Octane no está en uso.

Los jobs reciben **identificadores escalares** (`siteId`, `organizationId`), nunca modelos del dominio compartido: `SerializesModels` restaura los modelos antes de `handle()` y de cualquier middleware, así que un modelo con scope se rehidrataría sin contexto. Cuando el primer job real necesite contexto de organización, ese es el punto para construir el middleware/`Queue::after` — el job carga el `organizationId` en su constructor (nunca resuelve `Auth::user()`, inexistente en un worker) y una capa reutilizable fija/limpia `OrganizationContext` alrededor de `handle()`.
...
**Nota de secuencia (2026-09-18, decisión explícita del usuario):** la porción de "contexto resuelto en servidor y revalidado por request" de P4 se construyó (CLA-460 parcial) antes que P3 (`site_id` en el dominio compartido de Website). Es una excepción deliberada, no un abandono del orden estructura→contexto→autorización→enforcement: `OrganizationContext` resuelve directo desde `users.organization_id` (P2) y nunca toca `site_id`, así que no hay dependencia técnica real sobre P3. Este adelanto no se repite implícitamente para otras piezas de P3/P4/P5 sin la misma revisión explícita caso por caso.

**P4 queda completa (2026-09-18):** `AuditLogger` se resolvió con el selector auditado de `super_admin` (CLA-460 cont., `activity()` sin subject sobre `activity_log`). "Jobs" se cerró por verificación, no por código nuevo — ver D6: el reset `scoped` del `Worker` de Laravel ya está confirmado contra el vendor instalado, y sin ningún job consumidor real, un middleware/`Queue::after` propio sería scaffolding sin uso; se construye recién cuando el primer job que necesite contexto de organización lo justifique. Con esto, P4 (contexto real + auditoría) está completa; P5 (enforcement) sigue siendo la única fase activa sin cerrar, ya iniciada parcialmente por P5a (paneles/logins, CLA-460 cont.).
