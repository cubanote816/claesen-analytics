...
| ✅ Resuelto y cerrado (CLA-497, Done 2026-08-30, commit `ba149cc`): `MaintenanceRecordController::correctiveStats()`/`pendingClientReported()`/`clientReportedStatistics()` sin tenant-scope | Un `technician` (único rol interno sin `fieldops.view-all-clients`) veía agregados y PII (`contact_person`/`contact_phone`/`location_details`) de todos los clientes. Fix: `scopeForUser()` aplicado en la query de los 3 métodos; `hasBroadAccess()` (project_manager/admin/super_admin/etc.) sigue viendo todo por decisión vigente (CLA-377); `client` sigue bloqueado sin cambios. Tests dedicados en `MaintenanceRecordTenantScopeTest.php` (2 clientes reales, PII exacta con `assertDontSee` cruzado, shape exacto de respuesta vacía; para el actor amplio: `assertExactJson` sobre las dos respuestas agregadas globales de `correctiveStats()`/`clientReportedStatistics()`, y conteo + IDs + PII exacta por registro para `pendingClientReported()`, no `assertExactJson` en este último). Confirmado además con QA real contra dev (tokens Sanctum reales, fixtures borrados al terminar). Comiteado en `ba149cc` (`origin/main`/producción); step Deploy exitoso, `Verify deployment` falla por el falso negativo ya documentado de CLA-447. Smoke funcional productivo cross-tenant cubierto por waiver explícito del auditor (sin crear fixtures/credenciales en producción) |
| ✅ Implementado y testeado, pendiente de commit (CLA-498, 2026-09-07): `FieldOpsMediaController::store()` sin autorización tenant-aware (BOLA) | `POST /{modelType}/{modelId}/media` resolvía el modelo con `findOrFail()` sin ningún `Gate`/tenant check — un `technician` scoped a Client A podía subir media a equipo de Client B. Fix: ability `media()` nueva en `FieldOpsInfrastructurePolicy` (`canView() && can('fieldops.media')`) + `Gate::authorize('media', $model)` en el controller antes de `addMediaFromRequest()`; `show()`/`destroy()` no se tocaron (ya protegidos por el middleware vía `Media $media` con binding real). Gate serial 200/654 en verde (9 clases); hallazgo real durante la implementación: la migración 037 (CLA-496) siembra el rol real `technician` con `fieldops.media` ya concedido en cada `migrate:fresh`, así que el test que representa "technician sin ese permiso" tiene que revocarlo explícitamente del rol dentro de su propia transacción. QA real contra dev con tokens Sanctum reales (technician/financial_manager/admin) confirmó la matriz exacta y que ningún archivo persiste en los 403; fixtures borrados al terminar |
...
| 2026-08-30 | CLA-497 | Tenant-scope aplicado a `correctiveStats()`/`pendingClientReported()`/`clientReportedStatistics()`; `ClientReportedMaintenanceTest` corregido (actor sin `fieldops.view-all-clients` ya no asume acceso global); `MaintenanceRecordTenantScopeTest.php` nuevo (incluye `assertExactJson` agregado global para un actor amplio) + QA real contra dev con tokens Sanctum reales | Done — comiteado `ba149cc` (59/59, 347 assertions), en `origin/main` y producción, waiver aprobado para el smoke funcional productivo cross-tenant |
| 2026-09-07 | CLA-498 | Ability `media()` en `FieldOpsInfrastructurePolicy` + `Gate::authorize()` en `FieldOpsMediaController::store()`; `FieldOpsMediaTest.php`/`FieldOpsTenantAuthorizationTest.php` con tests de permiso faltante y cross-tenant; QA real contra dev con tokens Sanctum reales (technician/financial_manager/admin) | Implementado y testeado (200/654 assertions, 9/9 clases en verde), pendiente de commit/push/deploy y cierre en Linear |
