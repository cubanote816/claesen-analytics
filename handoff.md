...
> Última actualización: 2026-08-03 — **CLA-303** corrige la creación de Terrain desde un Complex: el mapa, el pin y los campos `lat`/`lng` parten de las coordenadas del Complex indicado por `complex_id`, con fallback al centro por defecto solo si ese Complex no está geolocalizado. Se añadió una regresión Livewire y las validaciones focales, lint PHP y `git diff --check` terminaron sin errores. Pendiente: commit dedicado y cierre del ticket en Linear.
