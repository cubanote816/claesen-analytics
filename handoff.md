...
> Última actualización: 2026-07-27 — CLA-278: segunda ocurrencia real del mismo bug de `wire:navigate`/`@push('scripts')`, esta vez en el canvas espacial de Luminaire Frame (`luminaire-frame-spatial-layout.blade.php`) — dejaba inerte el toggle Overview/Technical, el drag de marcadores y el botón "Add luminaire". Fix + de paso un bug de UX real: el canvas no mostraba la imagen del frame hasta tener al menos 1 luminaria colocada. Commit `9f2ef37` (sigue a `a935834`, mismo patrón). CLA-278 sigue In Progress — pendiente decidir si se cierra ya o se sigue sumando QA. **2/2 archivos auditados en vivo tenían este bug — sube prioridad de auditar los 4 location-pickers restantes, ver `docs/ai/known-risks.md`.**
