...
> Última actualización: 2026-07-06 — segundo incidente de producción resuelto: `SESSION_DOMAIN` estático generaba un conflicto real entre Filament (`backoffice.claesen.local`) y la PWA Safety/`service.claesen-verlichting` (Sanctum SPA sobre `*.claesen-verlichting.be`) — arreglar uno rompía el otro. Resuelto con middleware que resuelve el dominio de la cookie por host de la request, no por `.env` estático. El resto sigue igual: login de Azure OAuth roto tras deploy (`SESSION_DOMAIN` desincronizado + gap de opcache) resuelto antes en caliente; fix logo roto en email/PDF de inspecciones Safety, commiteado y pusheado (`c63c8d4`); ElectricalBoard, Media (FileLibrary) y Client encontrados igual de rotos que Terrain/Structure/Luminaire; los 3 arreglados y verificados en vivo, sigue sin commitear.
...
- **Deploy:** commiteado y desplegado a producción vía `deploy.sh` completo (no solo hotfix manual, ya que es un cambio de código real). Verificado en vivo: login Filament OAuth sigue funcionando y el submit de inspección desde Safety ya no da 419.
