...
> Última actualización: 2026-08-17 — **CLA-386** (In Progress): de la misma investigación de modelos IA salió la decisión de usar **Claude Sonnet 5** (no Gemini) para un MVP de identificación visual de luminarias en frames FieldOps — se acotó una propuesta muy amplia (microservicio Python, MCPs propios, embeddings visuales, reconstrucción 3D) a lo mínimo viable. Nuevo `Modules/Intelligence/Services/ClaudeVisionService.php` vía el SDK oficial `anthropic-ai/sdk` (Composer, `v0.42.0`), con `output_config.format` (JSON Schema estricto) pidiendo candidatos con `confidence`/`evidence`/`status: identified|probable|unknown` — "no identificado" es una respuesta válida y preferida, nunca se fuerza una identificación. Endpoint nuevo `POST /api/v1/fieldops/luminaire-frames/{frame}/vision-suggestions` (`LuminaireVisionController`, solo lectura, nunca crea/edita el frame). **Sin Docker en esta sesión** — no se pudo correr la suite completa contra MySQL. Sí se verificó: `php -l` en todos los archivos nuevos, `php artisan route:list` confirma la ruta registrada, y 2 tests sin dependencia de DB corrieron y pasaron en verde (`ClaudeVisionServiceTest`, abstención sin red cuando falta la API key o el catálogo está vacío). El test del controller (`LuminaireVisionControllerTest`, requiere `RefreshDatabase`) está escrito pero **no ejecutado** — falla localmente solo por falta de conexión a `mysql` (Docker), no por un error de lógica. `ANTHROPIC_API_KEY` quedó vacío en `.env`/`.env.remote` — cargar la key real antes de que el servicio funcione en runtime. Pendiente: correr `sail artisan test --filter=LuminaireVision` con Docker levantado.
