...
> Última actualización: 2026-07-08 — **Sistema de rediseño de UI de FieldOps completo**: los 6 patrones (Client/Complex CLA-230, Structure/Luminaire frame CLA-236, Terrain/Electrical board CLA-237, Luminaire propio CLA-238, Maintenance CLA-239, Catálogos CLA-240) implementados sobre `fieldops-backend-fixes`. **Gap conocido, sin cerrar todavía: `Complex` nunca recibió su propio patrón A** (mapa + backlinks de Electrical board + Terrains debajo) — solo se hizo `FoClient`, ver nota al final de la sección de detalle. El resto sigue igual: CLA-231/CLA-229 (Analytics) en revisión en `codex/instrumentacion-apps-internas`; `service.claesen-verlichting` es el frontend **FieldOps** (no Safety); Safety PWA real es `/home/totti/Claesen-Safety`; widget "Recent Safety Inspections" quitado; reorden de menú + bug de localización corregido; membrete corporativo + rebranding + bug de `MicrosoftGraphTransport`; segundo incidente de producción (`SESSION_DOMAIN`/Sanctum) resuelto.
...
- **Gap conocido, sin ticket todavía:** `Complex` nunca recibió su propio patrón A (mapa + backlinks de `electricalBoards` + `Terrains` debajo, tal como se propuso en el artifact v2) — solo se hizo `FoClient` (CLA-230). Encontrado a mitad del trabajo de Electrical board, documentado ahí mismo, pendiente de decidir con el usuario si se agrega como ticket nuevo.
- **Tests acumulados de todo el sistema:** `StructureFilamentTest`, `LuminaireFrameFilamentTest`, `TerrainFilamentTest`, `ElectricalBoardFilamentTest`, `LuminaireFilamentTest`, `FoMaintenanceFilamentTest` (ampliado), `CatalogFilamentTest`. FieldOps completo al cierre: **252/252 ✅**, verificado también en vivo contra la DB de dev real en cada paso.
