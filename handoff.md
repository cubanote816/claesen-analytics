...
> Última actualización: 2026-07-27 — CLA-278: cerrados los huecos de creación huérfana en la jerarquía FieldOps — `LuminaireFrame` ya no se puede crear sin al menos una Structure (máximo 2 frames por structure, regla de negocio real confirmada), y la segunda vía de creación de `Luminaire` (RelationManager dentro de Frame) ya no permite `serial_number`/`luminaire_type_id` vacíos ni posiciones duplicadas. Commit `f572bb7`. **Pendiente, ya planificado con el usuario:** reorganizar el menú lateral (sacar Structures/Luminaire frames/Luminaires del menú plano) + breadcrumbs jerárquicos con URLs anidadas (`Complexes > Stadion Bleukens > Terrains > ... > Luminaire frames`) — sin ticket propio todavía, próximo trabajo de la sesión.
