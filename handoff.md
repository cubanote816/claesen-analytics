...
> Última actualización: 2026-08-03 — **CLA-305** estabiliza la edición geográfica de Terrain: el mapa inicia con las coordenadas y el pin del registro, conserva la variante de su tipo, sincroniza el centro al mover el marcador y, tras guardar, refresca el formulario desde el modelo persistido. Playwright validó mover → guardar → mantener la nueva ubicación → recargar, usando datos temporales eliminados al finalizar. Lint PHP y `git diff --check` están limpios. Pendiente: commit dedicado y cierre del ticket en Linear.
