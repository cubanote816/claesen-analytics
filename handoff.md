...
> Última actualización: 2026-08-03 — **CLA-319** endurece dark/light mode de FieldOps: los cuatro location-pickers usan CSS nativo `.dark`, claves Alpine únicas y `@assets`/`@script` seguros para `wire:navigate`; se retiró el mecanismo frágil de `data-theme`/observer. También se ajustaron contrastes menores de la galería de luminarias y la advertencia de proximidad. Playwright verificó los cuatro mapas en light/dark y navegación SPA Electrical Board → Complex; se limpiaron los datos de QA. `ComplexEditLocationMapTest` pasó y `git diff --check` está limpio. Pendiente: commit dedicado y cierre de Linear.
