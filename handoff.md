...
>
> **CLA-406** (Todo, sin iniciar): el mismo diagnóstico encontró que el frontend desplegado en `service.claesen-verlichting.be` (`sbapu03:/var/www/service.claesen-verlichting.be/public`) tiene bundle con fecha **2026-08-17**, dos días antes del commit `bc3b389` (2026-08-19, el que trae el fix de `canAccessComplexes`/`canAccessClients` que permite a un `super_admin` ver esas secciones) — de ahí el menú incompleto que reportó el usuario. Más grave: el runner de GitHub Actions self-hosted conocido (`/opt/actions-runner-safety/` en prod-priv-01, repo `cubanote816/safety_claesen`) está parado en `74fa34f` (2026-07-21) y no coincide con ese bundle — el deploy del 17/8 no pasó por ningún pipeline documentado. Se encontraron **3 webroots independientes** en `sbapu03` para dominios FieldOps relacionados (`service.claesen-verlichting.be` 17/8, `safety.claesen-verlichting.be` 21/7, `fieldops.claesen-verlichting.be` 24/6), sin relación clara entre sí ni con el repo local auditado (`Claesen-Sport-updateing`, remoto `service.claesen-verlichting.git` — nombre distinto al del runner, sin confirmar todavía si son el mismo proyecto). Alcance: investigar el mecanismo de deploy real, documentar/normalizar el pipeline, y redeploy puntual del commit actual. Ningún cambio ejecutado en prod durante el diagnóstico — requiere plan + aprobación antes de tocar el webroot o cualquier pipeline. Detalle completo en `CLAUDE.md` sección "CLA-405 / CLA-406".
