...
> Última actualización: 2026-07-08 — CLA-235/CLA-236: sistema de rediseño de UI de FieldOps, primeros 2 patrones implementados (Structure, Luminaire frame) sobre la rama `fieldops-backend-fixes` (retomada tras interrupción de la sesión anterior — commits CLA-230/228/232/233/234 ya cerrados ese mismo día). Ver detalle completo abajo. El resto sigue igual: CLA-231/CLA-229 (Analytics) en revisión en `codex/instrumentacion-apps-internas`; `service.claesen-verlichting` es el frontend **FieldOps** (no Safety); Safety PWA real es `/home/totti/Claesen-Safety`; widget "Recent Safety Inspections" quitado; reorden de menú + bug de localización corregido; membrete corporativo + rebranding + bug de `MicrosoftGraphTransport`; segundo incidente de producción (`SESSION_DOMAIN`/Sanctum) resuelto.
...
- **Pendiente:** seguir con el resto del sistema de diseño — Terrain (patrón B-lite, 2 relaciones), Electrical board (patrón C, con el hueco real de attach/detach ya señalado), Luminaire propio (mini-canvas), Maintenance (patrón E, timeline), Catálogos (patrón F). Orden acordado con el usuario, confirmar antes de cada uno.
