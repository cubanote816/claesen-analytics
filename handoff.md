...
> Última actualización: 2026-07-07 — CLA-231: `resource_created`/`resource_updated` de Backoffice enganchados automáticamente vía `TrackableModelObserver` (sin tocar los ~30 recursos Filament existentes), en rama `codex/instrumentacion-apps-internas`, en revisión (sin GO técnico ni commit todavía). Además, corrección de memoria: `service.claesen-verlichting` es el frontend **FieldOps** (no Safety como se había registrado antes) — el Safety PWA real es `/home/totti/Claesen-Safety` (confirmado por el usuario). El resto sigue igual: CLA-229 (base de eventos) implementada; widget "Recent Safety Inspections" quitado de `/inspections`; reorden del menú lateral + bug de localización en `NavigationGroup`/`NavigationItem` corregido; membrete corporativo oficial en PDFs, rebranding "Claesen Intelligence Hub" → "Claesen Outdoor Lighting Platform", bug de `MicrosoftGraphTransport`; segundo incidente de producción (`SESSION_DOMAIN`/Sanctum) resuelto; ElectricalBoard, Media (FileLibrary) y Client arreglados y verificados en vivo, sigue sin commitear.

---

## Estado actual

- **Sprint activo:** CLA-229 + CLA-231 (Analytics — instrumentación de eventos), en revisión, ver detalle abajo. Además, sigue pendiente sin ticket Linear formal el "reto" exploratorio del usuario para portar `service.claesen-verlichting` (frontend legacy FieldOps, antes apuntaba a `api-claesen-sport-app`) contra el backend real `Modules/FieldOps`. Documentado acá por su tamaño, pero **sin commitear todavía** en ninguno de los dos repos (`claesen_api_web_oficial` y `service.claesen-verlichting`).
