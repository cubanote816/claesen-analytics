...
> Última actualización: 2026-07-04 (CLA-219 + CLA-221 + CLA-222 + CLA-223 + CLA-208(port) + CLA-225, panel Filament / Billing Control / Offer Simulator / sesion expirada)
...
- **CLA-224 (pendiente de commit, rama `issue-dashboard`):** siguiendo CLA-223, el usuario pidio sacar Offer Simulator del menu de navegacion en TODOS los entornos (no solo produccion) porque "parece codigo muerto". Implementado `shouldRegisterNavigation() -> false` (siempre) en `OfferSimulator.php` + test `test_navigation_item_is_never_registered`. **Interpretacion aplicada:** se oculta el item del menu, el codigo/pagina NO se borra ni se revierte de main — sigue alcanzable por URL directa para super_admin fuera de produccion (mismo gate de CLA-223). Cambios en working tree sin commitear todavia, esperando confirmacion final del usuario (se interrumpio el flujo por el hallazgo de CLA-208/225 mas abajo).
...
- **FO-010 / CLA-214 (código+tests listos, commit BLOQUEADO esperando GO explícito):** al planear el port del frontend de instalaciones a `Claesen-Sport`, se encontró que solo `GET /terrain-types` existía como catálogo — faltaban listados para `StructureType`, `AccessType`, `SafetyType`, `ElectricalBoardType`, `LuminaireFrameType`, `LuminaireType`, `LuminaireSubgroup` (necesarios para poblar selects en los formularios de alta). Implementado `CatalogController` (mismo patrón que `TerrainController@types`) + 7 `Resources` + 7 rutas GET + 8 tests nuevos (`CatalogEndpointsTest`), 181/181 FieldOps ✅. Vive en la rama **`fieldops-catalog-endpoints`** (creada a partir de `main`, en el worktree `claesen_api_dashboard_safety`) porque `main` ya estaba checked-out en el otro worktree (`claesen_api_web_oficial`, puerto 8000) y no se puede tener la misma rama en dos worktrees a la vez. El intento de `git commit` fue bloqueado por el clasificador de auto-mode: la aprobación del plan general no sustituye el "GO técnico" explícito que este `CLAUDE.md` exige como paso separado antes de cada commit. **Pendiente:** el usuario debe confirmar el GO para commitear esto.
- **Port frontend de instalaciones a `Claesen-Sport` (sin commitear, sin probar en navegador):** se construyó el dominio completo Complex→Terrain→Structure→(ElectricalBoard | LuminaireFrame→Luminaire) + adjuntos, consumiendo el contrato real de `Modules/FieldOps` (no la app legacy `service.claesen-verlichting`, que sirvió solo de referencia de UX). Detalle en memoria `project_fieldops_sport_port`. Verificado: `npm run build` limpio, contrato confirmado con curl+token real contra el backend, dev server transforma todos los módulos sin error — **no se hizo el recorrido visual en navegador** (sin herramienta de screenshot/browser disponible en el entorno). Fuera de alcance (sin API en Core): clientes, customer service, empleados, mantenimiento (bloqueado por `FO-009`).
- **Aclaración de infraestructura local — dos worktrees del mismo repo corriendo Sail en paralelo:** `claesen_api_web_oficial` (rama `main`) → puerto **8000**; `claesen_api_dashboard_safety` (esta sesión, rama `fieldops-catalog-endpoints`/`claesen_sport`) → puerto **8001**. Comparten la misma base de datos de dev. `Claesen-Sport/.env` (`VITE_API_URL`) se dejó apuntando a **8000** a pedido del usuario — implica que los 7 endpoints de catálogo de FO-010 devuelven 404 ahí hasta que se commiteen en `main` (puerto 8001 sí los tiene, sin commitear). Confirmar con el usuario cuál puerto usar antes de seguir probando formularios de alta.
- **Incidente menor:** durante la verificación se mató por error un proceso `vite` en el puerto 5173 que ya estaba corriendo antes de esta sesión (no arrancado por el asistente) — se avisó al usuario, no se confirmó su origen.
...
| Fecha | Ticket | Acción |
|-------|--------|--------|
| 2026-07-04 | FO-007 / CLA-212 | Done (spike) — confirmado que el dominio de Mantenimiento de luminarias del sistema anterior está vivo en producción (confirmación directa del usuario). No se cierra como N/A. Abierto `CLA-213`/FO-009 (Slice G) en Backlog para implementación futura. Bloquea FO-006. |
