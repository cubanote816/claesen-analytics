...
> Última actualización: 2026-09-01 — **Plan de despliegue Laravel 13 → producción, FASE 0** (auditoría de solo lectura; sin código, sin SSH, sin backups, sin deploy). `git fetch origin`: `release/laravel-13-rc1` local = `origin` = **`c6a5e89`**; `main` local = `origin` = **`ba149cc`** — cero divergencia; `origin/main` es ancestro estricto del release (24 commits / 0 merges) y no ha recibido ningún commit desde `ba149cc`. **PR #9** `OPEN`/`Draft`, base `main` ← `release/laravel-13-rc1@c6a5e89`, `MERGEABLE`/`CLEAN` — **NO mergear** (merge a `main` dispara `deploy.yml` = deploy productivo, gate único). **CI** run `33552786969` (head `c6a5e89`, `pull_request`) verde: `Static checks` + `Build front-end assets` + `PHPUnit (PHP 8.4)` = **1322 passed / 0 failed / 0 errors / 2 skipped** (4285 assertions); `composer audit --locked` sin advisories. **Gate del candidato: 1322/0/0/2 exacto.** Producción: `origin/main` en `ba149cc` (Laravel 12 esperado); el release **activo** en `prod-priv-01` **no está verificado** (requiere lectura remota autorizada). Linear: **CLA-514** `Backlog` (parent, nunca iniciado pese a subtickets de código Done), **CLA-525** `In Progress` (blockers antiguos CLA-521/522/524 resueltos, pero ahora `blockedBy` **CLA-530** y **CLA-531**), **CLA-523** `Backlog` (`blockedBy` CLA-525 y CLA-531), **CLA-529** `Done`. **Hallazgo central: no existe un entorno de staging Laravel** — `deploy.yml` es un job único a `prod-priv-01`; el único "staging" del repo es el sitio Astro estático (sbapu03), inservible para el backoffice. **2 tickets nuevos creados** (Backlog, parent CLA-514, priority High): **CLA-530** — aprovisionar staging Laravel real y separado de producción (12 criterios: host propio, PHP 8.4 + Imagick, MySQL 8.4 aislado, PHP-FPM, workers/scheduler Supervisor, dominio privado + `/up`, deploy que no escuche `push` a `main`, backups restaurables, rollback ensayable; `blocks` CLA-525); **CLA-531** — endurecer `deploy.sh`/`deploy.yml` (backup que aborta, sin `|| true` que oculte fallos de workers/scheduler, health check en-servidor con `about` + `/up` local + `supervisorctl status`) + rollback automático real probado en staging (`blocks` CLA-525 y CLA-523). **CLA-523 NO se amplió** — permanece `Backlog` bloqueado por CLA-525. Correcciones documentales de FASE 0 aplicadas a `laravel-13-staging-certification.md` / `known-risks.md` / `.github/workflows/tests.yml` (**solo el comentario**) / `context-map.md` / `laravel-13-readiness.md` / `CLAUDE.md`. Sin merge a `main`, staging, deploy, cambios de estado en los tickets ni `.codex/`.

> Última actualización: 2026-09-01 — **CLA-529** (**Done** en Linear, `completedAt` 2026-09-01T19:27:12Z, comentario de cierre `cf29afc6`; commits `a735ee9` fix + `4d0e32c`/`640242c` doc, rama `cubanote816/cla-529-mailing-ab-winner-y-follow-up-escriben-fechas-en-hora-local` sobre `0301aa4`, **empujados a `origin` e integrados en `release/laravel-13-rc1`** por fast-forward `--ff-only`, head certificado por la CI de cierre: `640242c`): cerrados los **11 fallos** que el waiver de CLA-528 dejó abiertos. Rama `cubanote816/cla-529-...` desde `0301aa4`. **Bug real de producción** (falla igual en L12, sin relación con L13): `ExecuteCampaignJob` escribe `ab_test_started_at`/`finished_at` con `now()` de la app (Brussels) pero `SelectAbWinnerCommand:37` / `DispatchFollowUpsCommand:42` comparaban con `DATE_SUB(NOW(), …)` de MySQL, cuyo `NOW()` sigue la tz de la **sesión** (puede diferir de `app.timezone`; reproducido con sesión en `+00:00` = gap 120 min en verano). **Fix — solo 2 archivos:** `$now = now()` único por ejecución + `DATE_SUB(NOW(), …)` → `DATE_SUB(?, …)` bindeado. **No** se tocó `DispatchScheduledCampaignsCommand`/`ExecuteCampaignJob`/casts/migraciones/`config/database.php`/transporte; claims/guards/audiencia/idempotencia intactos. Tests: `DispatchScheduledTest` 5 fixtures de `scheduled_at` → UTC explícito + assertions de `--dry-run`; `AbTestingTest`/`FollowUpTest` `tearDown()` que restaura `Carbon` + `time_zone`, +2 tests herméticos c/u (verano UTC+2 / invierno UTC+1, sesión pineada a `+00:00`, umbral ±1 s); nuevo `CampaignFormScheduledAtTimezoneTest` (`DB::table(...)` crudo, no el cast). **Riesgo DST residual documentado** (delay "N h" puede ser N±1 real; normalizar a UTC + backfill = alcance mayor, fuera de CLA-529). **`scheduled_at` no estaba afectado** (`CampaignForm` lo guarda ya en UTC). Verificación: 3 clases + form test 35/106; suite Mailing 197/0/0/2; **suite global desde `migrate:fresh`: `Tests: 1324, Failures: 0, Errors: 0, Skipped: 2` → 1322 passed**. Progresión CLA-528 waiver 1305/11/0/2 → **CLA-529 1322/0/0/2**. Pint idéntico a HEAD (deuda preexistente); form test limpio. **CI de PR #9 verde — 3ª corrida (run `33512861220`, head `4d0e32c`) y 4ª de cierre (run `33526166018`, head `640242c`), 2026-09-01:** ambas `Static checks` + `Build front-end assets` + `PHPUnit (PHP 8.4)` success — `Tests: 2 skipped, 1322 passed (4285 assertions)`, **0 fallos / 0 errores** (coincide con la corrida local; 1ª corrida de CI de toda la cadena L13 sin ningún fallo; los 11 de Mailing de `33396564409` eliminados). Sin deploy — antes de desplegar hay que verificar el reloj de sesión en staging/prod e inventariar A/B winners y follow-ups vencidos no reclamados. Comiteado `a735ee9` (fix) + `4d0e32c`/`640242c` (doc), los tres en `origin`; integrados en `release/laravel-13-rc1` (`0301aa4..640242c`, sin merge commit) y en `origin/release/laravel-13-rc1`. **CLA-529 cerrado en Linear como Done** (`completedAt` 2026-09-01T19:27:12Z, comentario `cf29afc6`, CI de cierre run `33526166018`). PR #9 sigue **Draft**; CLA-525 sigue **In Progress**. Sin merge a `main`, staging ni deploy. `.codex/` fuera.

> Última actualización: 2026-08-31 — **CLA-525 (cont.)** (In Progress): 1ª corrida real de la CI (`tests.yml`) contra `release/laravel-13-rc1` (Draft PR #9, **no mergear**). `Static checks` verde; `PHPUnit` **114 failed / 2 skipped / 1202 passed** — clasificados: **84** `ViteManifestNotFoundException` (el workflow no construía assets), **5+2** locale implícito (`AiTranslationTest`/`FieldOpsHierarchyNavigationTest`; CI usa `APP_LOCALE=en`, fixtures solo `nl`), **7** Google Geocoding sin clave ficticia (`GeocodingService` corta con `if (!$key) return null` antes de `Http::fake` — `GeocodingServiceTest` + `SyncComplexesFromRelationDeliveriesTest`), **5** Microsoft Graph `TypeError` (`MicrosoftGraphService::__construct` asigna `null` a `string $clientId` — `Laravel13CacheQueueMailCompatibilityTest` + `ExecuteCampaignJobCounterTest`), **11** Mailing dispatch conocidos (CLA-529). **Ninguna es regresión de código.** Fixes: `tests.yml` gana `actions/setup-node@v4` + `npm ci && npm run build`; herméticos solo en tests (locale `nl` en `setUp`; `config(['services.google_geocoding.key' => '<fake>'])`; `config(['mail.mailers.microsoft-graph.* => '<fake>'])`; mock de `MicrosoftGraphService` en el test de contrato Symfony). Sin `.env`, config productiva ni código de app. Verificado local: 6 clases 59/59; locale con `APP_LOCALE=en` 31/31; `npm run build` OK. **2ª CI — resultado real (run `33396564409`, commit `2e6aaf9`):** `Static checks` success, `Build front-end assets` success, `PHPUnit` **`11 failed / 2 skipped / 1305 passed` (4244 assertions, 235 s)**. Los 11 son **exclusivamente** Mailing/CLA-529 (`AbTestingTest` ×3, `DispatchScheduledTest` ×2, `FollowUpTest` ×6; grep `FAIL` → solo `Modules\Mailing`). **103 fallos de harness eliminados** entre la 1ª y 2ª corrida (84 Vite + 7 locale + 7 Geocoding + 5 Graph). **Baseline diferencial certificado — sin regresión de código por la cadena Laravel 13.** El job global sigue en `failure` (exit non-zero por los 11 conocidos) hasta que CLA-529 lleve el diferencial a cero. PR #9 sigue Draft, CLA-525 In Progress.

> Última actualización: 2026-08-30 — **CLA-528** (In Progress, pendiente de GO de commit): estabilización de la suite bajo Laravel 13. **Suite completa desde `migrate:fresh` (MySQL 8.4 aislado): `Tests: 1318, Assertions: 4244, Failures: 11, Errors: 0, Skipped: 2` → 1305 passed** (baseline CLA-520: 1086; cla-527 pre-cadena: 1127; **+219**). **1. `RoleAlreadyExists` (~160) — causa raíz:** solo 2 archivos (`ComplexViewAddressChipTest`, `ComplexEditLocationMapTest`) no usaban `RefreshDatabase` y commiteaban `super_admin`/usuarios/complejos sin rollback → cada `setUp()` posterior con `Role::create('super_admin')` reventaba (determinista por orden). Fix: `use RefreshDatabase` en los 2 + los 17 `Role::create(['name'=>'super_admin'])` → `firstOrCreate` (16 FieldOps + 1 Analytics). También arregla `FieldOpsTenantAuthorizationTest::admin_with_broad_permission_sees_every_client` (2 vs 5, pollution de `FoClient`). **2. Cascada Website (34 errores) — `migrate:rollback` roto, agravada por `f876942`:** `ConsultationEmailTest` + `MigrationJsonConversionTest` usaban `DatabaseMigrations`, cuyo teardown corre `migrate:rollback` de todo el batch → tropieza con `down()` rotos (`2026_04_09_124248_add_simulation_hash`: `1091 Can't DROP`) y deja el esquema medio desmontado para toda clase Website posterior. Antes de `f876942`, el `1553` abortaba el rollback más arriba y esas tablas quedaban intactas (`StaticSitePublishTest` 16 / `WorkDetailsTest` 3 / `GalleryMetadataJobTest` 1 pasaban en cla-527). Fix: los 2 → trait **`DatabaseTruncation`** (migra hacia adelante 1×, trunca entre tests, `afterCommit` sigue disparando, **nunca** corre `migrate:rollback`). **3. 3 bugs de test residuales:** `WorkDetailsTest::work_story_returns_nl_translation` (`SetPanelLocale` pisa `setLocale` → header `Accept-Language: nl`); `ClientPortalInfrastructureTest` (`topology()` 2× duplica `fo_terrain_types.code` → `firstWhere ?? factory`); `MicrosoftAuthRedirectTest::test_module_config_merge_is_skipped_when_config_is_cached` (harness mal planteado — `configurationIsCached()` lee el binding `config_loaded_from_cache` del bootstrap, igual en L12.50 y L13; el `file_put_contents` del cache no lo invalida → fix: `$this->app->instance('config_loaded_from_cache', true)`; **no** es regresión L13). **Fuera de alcance:** las 11 restantes = Mailing dispatch (`AbTestingTest` ×3, `DispatchScheduledTest` ×2, `FollowUpTest` ×6, `ExecuteCampaignJob was not pushed` / `null is not null`) — bug real de relojes (escritura hora local vs comparación UTC), impacto productivo, → **CLA-529** (Backlog); y los `down()` rotos de las migraciones `offer_simulations` (deuda de rollback → CLA-523). Verificado: orden problemático 101/101 (0 `RoleAlreadyExists`); 2 triggers `DatabaseTruncation` + 4 downstream Website 50/50; `MicrosoftAuthRedirectTest` 4/4; suite completa 1305 passed / 0 errores. Commit dedicado a hacer en rama `cla-527` tras `f876942`. Sin push ni deploy.

> Última actualización: 2026-08-30 — **CLA-523 (parcial)**: corregido el `down()` de `Modules/FieldOps/database/migrations/2026_07_21_000003_create_luminaire_positions_and_replacement_history.php` — dropeaba el índice único `fo_luminaires_one_active_per_position` antes de la FK `active_position_id` que MySQL 8 usa como respaldo → `SQLSTATE 1553 Cannot drop index ... needed in a foreign key constraint`, abortando `migrate:rollback`. Nuevo orden en el bloque `Schema::table('fo_luminaires', ...)`: `dropForeign(['active_position_id'])` → `dropUnique(...)` → `dropColumn('active_position_id')`. Resto del `down()` intacto. Verificado end-to-end contra MySQL 8.4 aislado (`testing_l13_dn`): `migrate` 179 → `migrate:rollback --step=15` (incluye `2026_07_21_000003`, **exit 0 sin 1553**) → columnas/índice/tabla `fo_luminaire_positions` limpiamente eliminados → `migrate` de nuevo exit 0. Elimina la familia ~14 fallos `1553` de la suite y, por cascada, los ~12 `Unknown column 'attribute_changes'` de `ConsultationReminderCommandTest`. Pint idéntico a HEAD (deuda de estilo del archivo, no del diff). CLA-523 permanece en `Backlog` (estado real en Linear; ensayo de rollback + health check + staging = infra real). Comiteado en la punta de la cadena stacked (rama `cla-527`). Sin push ni deploy. `.codex/` sin tocar.

> Última actualización: 2026-08-30 — **Suite completa ejecutada localmente** (rama `cla-527`, toda la cadena L13): `./vendor/bin/phpunit` completo contra MySQL 8.4 aislado desde `migrate:fresh` → **1318 tests, 174 errores, 15 fallos, 2 skipped (1127 passed)**. Baseline CLA-520: 1086/186/2. **Errores+fallos 189 vs 186 — plano, sin regresión, ninguna familia nueva.** Clasificación completa en `CLAUDE.md` §CLA-525: ~160 `RoleAlreadyExists` (harness FieldOps `setUp` con `Role::create`, preexistente desde el baseline L12), ~14 MySQL 1553 `Cannot drop index fo_luminaires_one_active_per_position` (ciclos `migrate:fresh` de tests Website; un `down()` dropea un índice aún referenciado por FK — **relevante solo para el rollback de CLA-523**), ~12 `attribute_changes` (cascada del 1553, NO regresión de CLA-526), ~12 Mailing dispatch (preexistentes), + 4 de las familias config-cache/locale/tenant-count/ClientPortal-factory. **Ningún test rompe por CLA-519/526/517/516/521/522/524/527.** Follow-ups: (1) orden `drop FK → drop index` en el `down()` roto → CLA-523; (2) normalizar `setUp` de la batería FieldOps a `Role::firstOrCreate` + `forgetCachedPermissions` → CLA-525 (ítem "CI en verde"). El host de dev quedó con `pdo_sqlsrv`/`msodbcsql18` instalados para inspección local de Performance/Cafca (setup de host, no toca el repo). Sin push ni deploy.

> Última actualización: 2026-08-30 — **CLA-527** (cierre técnico aprobado): unificado el casing de directorios de migración de módulo. `FieldOps` y `Website` usaban `Database/{Migrations,Factories,Seeders}` (mayúscula); renombrados a minúscula vía `git mv` (~96 renames, timestamps intactos → orden de migración idéntico). `Modules/Safety/Database/Seeders/*` (3 seeders) fusionados a `database/seeders/`, resolviendo de paso un `SafetyDatabaseSeeder` duplicado (stub muerto borrado, el real conservado — verificado por reflexión). `composer.json`: mapeos PSR-4 explícitos añadidos para `Modules\{FieldOps,Website,Safety}\Database\*` → rutas minúscula (namespaces `Database\` studly sin cambio). `FieldOpsServiceProvider`/`WebsiteServiceProvider` + 3 tests con rutas hardcodeadas corregidos. Post-rename los 11 módulos son consistentes (auto-discover encuentra a todos + `loadMigrationsFrom` manual a la misma ruta → migrator deduplica, sin doble registro). Verificado: `composer validate --strict` OK, `dump-autoload` 12712 clases todas resolviendo desde minúscula, `migrate` completo 179 migraciones / 0 duplicados / mismo count que CLA-526, `module:list` 11/11, `route:list` sin regresión. Assets Filament regenerados por `post-autoload-dump` NO comiteados. `docs/ai/known-risks.md` entrada marcada resuelta. Commit dedicado autorizado; sin push ni deploy. `.codex/` sin tocar.

> Última actualización: 2026-08-30 — **CLA-525 / CLA-523** (artefactos preparados, ejecución pendiente de infra real): son tickets de ejecución contra infraestructura real (SSH a prod-priv-01/sbapu03, deploy a staging, E2E por navegador, ensayo de rollback en servidores reales) — no cerrables desde el entorno de desarrollo. Estado real en Linear (verificado 2026-08-31): **CLA-525 = `In Progress`**, **CLA-523 = `Backlog`** (nunca iniciado). **Preparado:** `.github/workflows/tests.yml` (CI de PHPUnit sobre PHP 8.4 + mismas extensiones que staging, servicio MySQL 8.4, `migrate --force` desde cero, `php artisan test` + `composer validate/audit`; no es gate de merge duro todavía — su primera corrida establece el conteo diferencial vs baseline CLA-520 1086/186/2) y `docs/ai/laravel-13-staging-certification.md` (checklist E2E por rol super_admin/admin/project_manager/technician/viewer/client + subsistemas + ensayo de rollback calcado de `deploy.sh` + rediseño del health check `curl backoffice.claesen.local` → check en-servidor a `/up` + `supervisorctl status`). **Pendiente de ejecución:** correr la CI, desplegar el candidato en staging (rama de release consolidada = merge de la cadena stacked cla-519→…→cla-524), matriz E2E, ensayo de rollback, aplicar el health check. Commit dedicado autorizado; sin push ni deploy.

> Última actualización: 2026-08-30 — **CLA-524** (cierre técnico aprobado): PHPUnit 12 + estabilización de la suite. PHPUnit 12.5.34 + Collision 8.9.5 ya estaban instalados (CLA-519); `phpunit.xml` ya era moderno. **Dos regresiones de cobertura silenciosas del upgrade corregidas** (PHPUnit 12 dejó de descubrir metadata en doc-comments): `tests/Feature/LocalizationTest.php` (4 tests con `/** @test */` que no corrían — reescrito con `#[Test]`/`#[DataProvider]` y contra `BrowserLocaleMiddleware` directo, cubriendo en/nl/fr/de + fallbacks) y `MirrorSyncStatusPageManualSyncTest.php` (`/** @dataProvider */` → `ArgumentCountError`, fix `#[DataProvider(...)]`, recupera las 4 filas). **3 firmas con deprecations PHP 8.4** (`ComplianceControllerTest`, `NotifyInactiveManagersCommandTest` — `?string`/`?Carbon` sin relajar aserciones). `tests/Feature/ExampleTest.php` (scaffold, `GET /`→200 que nunca se cumplió) reescrito para afirmar el redirect guest→login real. `phpunit.xml` endurecido: `failOnEmptyTestSuite`/`failOnWarning` + `displayDetailsOn*` (no `failOnDeprecation`/`failOnRisky` — hay ~186-200 fallos preexistentes y el gate de suite verde es CLA-525). Test nuevo `Laravel13MigrationsCompatibilityTest.php` 3/3 (protege el fallback `config('activitylog.table_name', 'activity_log')` de CLA-526 + reversibilidad de la migración v5). Clasificación de familias de fallos (preexistentes vs regresiones L13 ya corregidas en la cadena) documentada en CLAUDE.md para el gate de CLA-525. Verificado: suites raíz `Unit`+`Feature` 32 tests / 0 fallos tras los fixes; 7 `Laravel13*CompatibilityTest` verdes; 0 deprecations en los archivos tocados. WAIVER diferencial (= toda la cadena L13): suite completa sobre PHP 8.4 + alineación de versiones CI↔staging → CLA-525. Commit dedicado autorizado; sin push ni deploy. `.codex/` sin tocar.

> Última actualización: 2026-08-30 — **CLA-522** (cierre técnico aprobado): validación de cache / queues / scheduler / mail / DB bajo Laravel 13 + Symfony 8.1.5. **Bug real corregido:** `cache.serializable_classes => false` de CLA-519 rompía `EmployeeDashboardRankingService::getTopEmployees()` — cacheaba un `Illuminate\Support\Collection` (con otro `Collection` anidado en `rankings`) que en L13 `DatabaseStore::get()` deserializa como `__PHP_Incomplete_Class` (`unserialize(..., ['allowed_classes' => false])`); antes de CLA-519 la clave no existía y funcionaba. Regresión latente (sin desplegar). Fix: cachear la forma array (`->computeTopEmployees(...)->toArray()`, aplana el `Collection` anidado) y rehidratar a `Collection` a la salida — `config/cache.php` se mantiene en `serializable_classes => false`. Los otros 9 `Cache::` del repo son seguros (auditados: escalares/arrays/tokens). Scheduler `schedule:list` 0 duplicados. Queues `sync`, jobs con `SerializesModels` estándar (serialización de cola independiente de `cache.serializable_classes`). `MicrosoftGraphTransport` extiende `Symfony\...\AbstractTransport` — API estable 7→8. `ReadOnlyTrait` (guard SQL Server read-only) PHP puro, sin impacto L13. Verificado: `EmployeeRankingsTest` 9/9 (incl. test nuevo de round-trip contra cache `database` real), `Laravel13CacheQueueMailCompatibilityTest` 3/3, `MicrosoftGraphTransportPayloadTest` 1/1; suite Mailing amplia 191 tests / 11 fallos = el conjunto preexistente documentado (`AbTestingTest`/`DispatchScheduledTest`/`FollowUpTest`, dispatch de jobs en cola, cero solapamiento con el cambio de `Modules/Employee`). WAIVER diferencial (= CLA-519/526/517/516/521): worker de cola real + envío real de email + SQL Server real → CLA-524/CLA-525. Commit dedicado autorizado; sin push ni deploy. `.codex/` sin tocar.

> Última actualización: 2026-08-30 — **CLA-521** (cierre técnico aprobado): certificación de auth / sesiones / CSRF / Sanctum multidominio. **El único cambio de L13 en toda esta capa es `VerifyCsrfToken`/`ValidateCsrfToken` → `PreventRequestForgery`, ya aplicado en CLA-519** — este ticket no cambia `composer.json`/`composer.lock` ni código de aplicación. `PreventRequestForgery` es un drop-in retrocompatible (verificado por reflexión: `$originOnly=false` → nunca lanza `OriginMismatchException`; `$allowSameSite=false` → same-site sigue necesitando token; `$neverVerify=[]`), y grep confirma que la app nunca opta por los modos estrictos (`::allowSameSite()`/`::useOriginOnly()`). `ResolveSessionCookieDomain`/`statefulApi()`/`EnsureFrontendRequestsAreStateful` sin impacto de L13; Sanctum 4.3.3 / Socialite 5.30.1 / microsoft-azure 5.2.1 ya funcionando. Sesión sigue en serialización `php` (CLA-519), sin invalidación. Verificado: suite de auth de `Modules/Core` vía `phpunit` **67/68** (`ClientPortalLoginTest`, `SportLoginTest`, `Change/Reset/SetupPassword`, `SessionKeeperTest`, `MicrosoftAuthRoleGateTest`, `ResolveSessionCookieDomainTest` 7/7, `PanelAccessTest` 10/10) — el único fallo es el preexistente conocido `test_module_config_merge_is_skipped_when_config_is_cached` (timing del harness, falla idéntico en L12). Test nuevo `Modules/Core/tests/Feature/Laravel13AuthCsrfCompatibilityTest.php` 3/3. El 419 no se testea como feature (guard `runningUnitTests()` del middleware corta CSRF en test por diseño). WAIVER diferencial (= CLA-519/526/517/516): E2E por navegador contra dominios reales de staging (Azure OAuth, doble-cookie bajo túnel, SameSite/Secure/Domain/Origin/Referer, modal 419) → CLA-525. Commit dedicado autorizado; sin push ni deploy. `.codex/` sin tocar.

> Última actualización: 2026-08-30 — **CLA-516** (cierre técnico aprobado): certificación de Filament 5.7 / Livewire 4.4 / Media Library. Filament 5.7.6, Livewire 4.4.2 y Spatie MediaLibrary 11.23.5 ya estaban en su sitio (CLA-520/518). **Único cambio real:** `filament/spatie-laravel-media-library-plugin` **3.3.30 → 5.7.6** (aplicado en la lock de CLA-519) — antes convivía el plugin de la línea 3 (Filament 3) con Filament 5.7.6, un emparejamiento incorrecto que funcionaba por accidente; v5.7.x exige `filament/support: self.version`, así que ahora quedan bloqueados en la misma versión. `SpatieMediaLibraryFileUpload` no cambió de namespace ni de API — los 7 recursos que lo usan (`ProjectResource` con closures `saveRelationshipsUsing`, FieldOps `Complex/Terrain/Structure/Luminaire/ElectricalBoard`, Cafca `EmployeeForm`) no requieren cambios de código. **Este ticket no cambia `composer.json`/`composer.lock` ni código de aplicación.** Verificado: `filament:upgrade`/`filament:optimize` OK, 34/34 recursos del panel admin resuelven sin excepción, las 23 firmas de `SpatieMediaLibraryFileUpload` que usa la app existen en v5.7.6. Test nuevo `Modules/Core/tests/Feature/Laravel13FilamentMediaCompatibilityTest.php` 4/4. Suite Filament vía `phpunit` directo: `FieldOpsMediaTest` 20/20, `CatalogFilamentTest` 17/17, `Complex/Luminaire/Structure/ElectricalBoard/Terrain FilamentTest` 43/43 — **80 tests, 346 aserciones, 0 fallos**. Las 4 "deprecations" del runner de Pest son PHP 8.4 implicit-nullable en helpers de test de Safety (no relacionadas con Filament/media, → CLA-524). WAIVER diferencial (= CLA-519/526/517): E2E por navegador + suite completa → CLA-524/CLA-525. Commit dedicado autorizado; sin push ni deploy. `.codex/` sin tocar.

> Última actualización: 2026-08-30 — **CLA-527** (Backlog, Low): hallazgo de CLA-517 — casing inconsistente de los directorios de migración de módulo (`FieldOps`/`Website` usan `Database/Migrations`, los otros 7 `database/migrations`). Sin bug observable hoy (`migrate` limpio, `migrate:status` sin duplicados), riesgo latente si se quita el `loadMigrationsFrom` manual de FieldOps/Website. Documentado en `docs/ai/known-risks.md` → "Deuda técnica" (commit `0090725`).

> Última actualización: 2026-08-30 — **CLA-517** (cierre técnico aprobado): certificación de `nwidart/laravel-modules` 13. **Bump puro de compatibilidad con Laravel 13** — el diff `v12.0.5..v13.0.0` toca solo 3 archivos (CI, README, constraints de `composer.json`); cero cambios de código/config/API/discovery/`module.json`. El paquete ya quedó en `v13.0.0` con el corte de CLA-519, y `config/modules.php` ya coincide con el default de v13 (prep de CLA-518). **Este ticket no cambia `composer.json`/`composer.lock` ni código de aplicación.** Evidencia: `module:list` 11/11 `[Enabled]`, `route:list` con rutas modulares (`api.cafca.index` resuelve), `list` con 33 comandos de módulo, `schedule:list` sin duplicados, `deploy.sh` sin referencias a `modules_statuses.json`/`module:enable/disable`, migraciones modulares OK (incluidas en el `migrate` completo exit 0 de CLA-526). Test nuevo `Modules/Core/tests/Feature/Laravel13ModulesCompatibilityTest.php` 6/6. Hallazgo colateral (deuda preexistente, no de v13, fuera de alcance): casing mixto `Database/Migrations` (FieldOps/Website) vs `database/migrations` (los otros 9) — sin doble registro, `migrate` limpio. WAIVER diferencial (= CLA-519/526): suite PHPUnit completa → CLA-524/CLA-525. Commit dedicado autorizado; sin push ni deploy. `.codex/` sin tocar.

> Última actualización: 2026-08-30 — **CLA-526** (cierre técnico y waiver diferencial aprobados): Spatie majors + datos, segundo eslabón tras CLA-519. El corte de `composer.lock` de CLA-519 ya instaló Activitylog 5.1.0, Query Builder 7.3.3, Permission 8.3.0 y Translatable 6.14.1; este ticket cierra las roturas funcionales. **Activitylog 5:** nueva migración reversible `2026_08_30_100000_upgrade_activity_log_table_to_activitylog_v5` (añade `attribute_changes` json, mueve `properties->attributes/old` ahí, dropea `batch_uuid`) — sin ella cada create/update/delete de `ConsultationRequest` (único modelo `LogsActivity`) lanzaba `Unknown column 'attribute_changes'`; + patch mínimo a las 3 migraciones históricas `2026_02_09_14034*` (`config('activitylog.table_name', 'activity_log')` — la clave fue eliminada del config v5 y resolvía a `null` → `Schema::create(null)` en `migrate:fresh`); + `config/activitylog.php` publicado (forma v5, env var `ACTIVITYLOG_ENABLED`). **Query Builder 7:** `PortfolioService::allowedFilters([...])` → variádico `allowedFilters(...)` (único consumidor de `Spatie\QueryBuilder`). **Permission 8 / Translatable 6.14:** sin cambios de código — API estable; polish en `config/permission.php` (`models.team`/`models.default_model` => null). Verificado: `composer validate --strict`, `migrate` completo sobre DB vacía (179 migraciones, exit 0) + ciclo `migrate:rollback --step=1 → migrate` de la migración v5, smokes `tinker` de los 4 frentes contra `testing_cla526` real, `PortfolioApiTest` 10/10 (filtro QB7 + traducciones), `MicrosoftAuthRoleGateTest` 5/5 + `PanelAccessTest` 10/10 (RBAC bajo Permission 8). **WAIVER diferencial (= CLA-519):** la suite PHPUnit completa con `RefreshDatabase` no se certifica localmente (`migrate:fresh` sobre el MySQL reenviado se reinicia antes de terminar); matriz RBAC completa por rol + suite entera → CLA-524/CLA-525. `testing_cla526` eliminada, sin procesos huérfanos. Commit dedicado autorizado; sin push ni deploy. `.codex/` sin tocar. Con CLA-526 cerrado, CLA-524 y CLA-525 dejan de estar bloqueados por Spatie.

> Última actualización: 2026-08-30 — **CLA-519** (cierre técnico y waiver diferencial aprobados): **el repositorio corre ahora Laravel 13.29.0**. Corte atómico de `composer.json`/`composer.lock` (177 paquetes; 107 updates / 2 installs / 4 removes, idéntico al sandbox de CLA-518): `php ^8.4`, `laravel/framework ^13.0`, Tinker 3, Modules 13, Activitylog 5, Permission 8, Query Builder 7, Translatable 6.14, Resend 1.4, Filament 5.7 + media 5.7, PHPUnit 12, Pail 1.2.7, Sail 1.67, Collision 8.9.5; Filament/Livewire/Sanctum/MediaLibrary conservan su major. Adaptación de núcleo: CSRF `ValidateCsrfToken` → `PreventRequestForgery` en `config/sanctum.php` + `AdminPanelProvider.php` (drop-in retrocompatible — `hasValidOrigin`/Sec-Fetch-Site es vía aditiva, sin `OriginMismatchException` con defaults, flujo de token SPA/túnel intacto); `config/cache.php` → `'serializable_classes' => false` (idéntico al skeleton L13); `config/session.php` → `'serialization' => 'php'` (desviación deliberada del nuevo default `'json'` para no cerrar sesiones activas — migración JSON planificada aparte). Puente mínimo Activitylog 5 en `Modules/Website/Models/ConsultationRequest.php` (único consumidor v4 en el repo) — solo restaura el boot; la migración de `activity_log` + datos + tests siguen en **CLA-526**, que pasa a ser dependencia dura. Test nuevo `Modules/Core/tests/Feature/Laravel13CoreCompatibilityTest.php` 4/4. Verde: `composer validate --strict`, `check-platform-reqs --lock`, `composer audit --locked` (0 advisories), `config/route/event/view:cache`, `package:discover`, `about`, `schedule:list` (24 comandos), tests focalizados sin DB 11/11 (`ResolveSessionCookieDomainTest` 7/7). `MicrosoftAuthRedirectTest` 3/4 con el único fallo preexistente conocido del baseline (`module config merge`, timing del harness). **WAIVER diferencial aprobado:** la suite con base de datos no se pudo certificar localmente (MySQL solo accesible por puerto reenviado del checkout principal; `migrate:fresh` de 178 migraciones se mata antes de terminar — mismo problema de entorno que CLA-515); DB aislada `testing_cla519` ya eliminada. La certificación de la suite completa sobre PHP 8.4 / Laravel 13 corresponde a **CLA-524 + CLA-525**. Commit dedicado autorizado; sin push ni deploy. `.codex/` sin tocar.

> Última actualización: 2026-08-30 — **CLA-518** (cierre técnico y waiver aprobados): matriz de compatibilidad para Laravel 13 definida sin modificar `composer.json`, `composer.lock`, aplicación ni base de datos. El sandbox aislado resolvió e instaló Laravel 13.29.0 con 177 paquetes (2 instalaciones, 107 actualizaciones, 4 eliminaciones); `why-not` objetivo, validación estricta, requisitos de plataforma y audits pasan. Ruta coordinada: PHP `^8.4`, Tinker 3, plugin Filament Media 5.7, Laravel Modules 13, Activitylog 5, Permission 8, Query Builder 7 y PHPUnit 12; Filament/Livewire/Sanctum/MediaLibrary conservan sus majors. Bloqueante crítico: Activitylog 5 exige PHP 8.4 y migración reversible de `activity_log` bajo CLA-526. Antes de CLA-519 se debe integrar todo el trabajo Laravel 12 pendiente, congelar dependencias/bootstrap y repetir el solver sobre la rama base final. Ownership y rollback quedan en `docs/ai/laravel-13-compatibility-matrix.md`. **WAIVER aprobado por GO técnico:** al ser diagnóstico/documentación, no se ejecutó la suite; cobertura alternativa = solver, instalación limpia, validate, platform check y audits. Commit dedicado autorizado; sin push ni deploy.

> Última actualización: 2026-08-29 — **CLA-520** (🚧 Linear In Progress; cierre técnico y waiver diferencial aprobados): saneamiento Composer manteniendo Laravel 12. `composer.lock` actualiza 34 paquetes en tres lotes controlados, sin saltos de major ni cambios en `composer.json`: Laravel 12.68.0, Filament 5.7.6, Livewire 4.4.2, Sanctum 4.3.3, Dompdf 3.1.6, MediaLibrary 11.23.5 y transitivas vulnerables. `composer audit` pasa con 0 advisories; `composer validate --strict`, platform requirements e instalación limpia aislada de 179 paquetes pasan. Filament/Livewire endurecieron escape/serialización HTML y se adaptaron cuatro tests FieldOps, sin código de aplicación. Build Vite, Artisan, scheduler, permisos, aislamiento, media, PDF y payload Microsoft Graph verificados. Suite diferencial completa en `testing_cla520`: **1086 passed, 186 failed, 2 skipped (3270 assertions, 1131.20 s)** frente a 1072/200/2 de CLA-515; las familias restantes son las mismas preexistentes (Example/locale-config, estado compartido FieldOps, Mailing y rollback Website). Commit dedicado autorizado; sin push ni deploy. Despliegue/observación siguen reservados para CLA-523 tras CLA-525, por lo que CLA-520 permanece abierto.

> Última actualización: 2026-08-29 — **CLA-515** (✅ Done): baseline/readiness para Laravel 13. `composer.json` exige PHP `^8.3`; el host usa PHP 8.4.24 y Composer oficial 2.10.3 sin deprecations; `lara-zeus/spatie-translatable` quedó fijado en `^2.0` (lock 2.0.1); Sail y `monitor.sh` quedaron alineados a PHP 8.4. La matriz y evidencia viven en `docs/ai/laravel-13-readiness.md`. PASS: validación Composer estricta, instalación limpia de 179 paquetes desde lock, platform requirements, migraciones desde cero en `testing_cla515`, build Vite, npm audit, Artisan/scheduler y checks de scripts/Compose. Baseline completo histórico: **1072 passed, 200 failed, 2 skipped (3232 assertions, 2223.18 s)**; los fallos preexistentes se concentran en aislamiento FieldOps, locale/config, Mailing y rollback lento de migraciones Website. `Imagick` 3.8.1 ya está cargado en el host y los dos tests de media afectados pasan 2/2 (17 assertions); Sail PHP 8.4.17 también lo carga. Producción verificada directamente: CLI 8.4.22, Composer 2.10.1, FPM 8.4 activo, dos workers Redis y scheduler RUNNING sobre `/usr/bin/php8.4`. `backoffice.claesen.local` es solo LAN y las demás apps llegan por túnel; no debe exponerse a Internet. Run 106 completó el deploy, pero su health check a `192.168.254.52:443` recibe connection refused y deja el workflow en rojo; sustituirlo en CLA-523 por un check acorde al listener/túnel interno. `composer audit` mantiene 60 advisories/21 paquetes para CLA-520. CI de tests/staging quedan explícitamente para CLA-524/CLA-525. Sin cambios de aplicación, secretos, push ni deploy.

> Última actualización: 2026-08-29 — **CLA-513** (✅ Done): actualizado únicamente `package-lock.json` con `npm audit fix` sin `--force`, manteniendo intactos los rangos declarados en `package.json`. Dependencias directas relevantes resueltas a Axios `1.20.0`, Concurrently `9.2.4` y Vite `7.3.6`; transitivas vulnerables (`shell-quote`, `follow-redirects`, `form-data`, `nanoid`, `picomatch`, `postcss`, `rollup`) quedaron en versiones parcheadas. El cambio de nombre raíz del lockfile a `claesen_api_opportunities_module`, generado previamente por el `npm install` del usuario, se preservó y quedó declarado. Verificación: `npm audit --audit-level=moderate` → 0 vulnerabilidades; `npm ls --all` → árbol resuelto (solo dependencias opcionales de otras plataformas ausentes, esperado); `npm run build` → PASS con Vite 7.3.6; `git diff --check` → PASS. Sin cambios en `package.json`, código de aplicación, secretos, push ni deploy. Test Gate cerrado con waiver aprobado por el auditor: actualización exclusiva de lockfile sin suite JS configurada; cobertura alternativa mediante audit, resolución completa del árbol y build de producción.

> Última actualización: 2026-08-28 — **CLA-448** (🚧 In Progress): auditoría de higiene de repo tras publicar la integración de CLA-404/439/440/444/445 a `main` — `.gitignore` raíz ya tenía `/tmp` (línea 21) y funciona bien para archivos nuevos, pero 5 archivos quedaron trackeados desde antes de esa regla (`tmp/.gitignore` —copia idéntica del `.gitignore` raíz, probable error—, `tmp/fieldops-backoffice-map-panels-v1.html`, `tmp/fieldops-map-ui-system-v3.html`, `tmp/image.png`, `tmp/ttt.md`), confirmado con `git check-ignore -v` devolviendo "no ignorado" mientras seguían en el índice. Sin referencias de código a ninguno (solo 2 menciones narrativas en este mismo `handoff.md`, no un link a blob). Fix: `git rm --cached` con rutas explícitas sobre los 5 (nunca `-r tmp/`) — copias en disco intactas, tamaño/sha256 verificados idénticos antes/después; `git ls-files tmp` queda vacío; `git check-ignore -v tmp/image.png` ahora sí matchea la regla `/tmp`. Sin tocar `.gitignore`, `config/cors.php`, `docs/export/`, el archivo suelto `main` del worktree, ni nada de frontend. Commit separado de la integración ya publicada, pendiente de GO técnico para stagear `CLAUDE.md`/`handoff.md` y comitear. Detalle en `CLAUDE.md` sección "CLA-448".

> Última actualización: 2026-08-27 — **CLA-445** (✅ Done): auditoría rápida pedida por el usuario tras CLA-444 encontró que la nota de cierre de CLA-440/CLA-444 ("corregido a `gpt-5.4-nano-2026-03-17`") era solo parcial — `config/services.php`/`.env.example` sí quedaron en nano, pero el fallback en código de `OpenAiImageGenerationService::requestName()` (activo solo cuando la key `services.openai.name_model` está ausente de la config, no cuando falta la env var) y el mock del test seguían en `gpt-5.4-mini-2026-03-17`, igual que `docs/ai/context-map.md`. Fix mecánico de 2 líneas + test nuevo dedicado (`test_it_sends_the_configured_model_in_the_naming_request`, filtra exclusivamente la request a `chat/completions`, sin mezclar con las aserciones de `images/edits`) — hasta este ticket ningún test verificaba el `model` real enviado en la llamada de naming. Sin `.env`, frontend, Gemini, generación de imagen ni prompts tocados; sin llamadas reales a OpenAI. 19/19 tests en verde. Pint reporta 2 issues preexistentes (no introducidos, no corregidos, confirmados idénticos contra `HEAD`). Detalle en `CLAUDE.md` sección "CLA-445".

> Última actualización: 2026-08-27 — **CLA-404** (✅ Done, cierra el ticket que había quedado parcial el 2026-08-25): último punto pendiente — banner de frescura de datos en el sub-paso "Project" de la PWA Safety (`Claesen-Safety`). `GET /api/v1/safety/projects` devuelve `{data, meta.last_synced_at}` (finished_at de la corrida `completed` más reciente de `MirrorSyncRun`, `null` si ninguna completó nunca; corridas failed/running nunca cuentan aunque sean más recientes — test reforzado para probarlo con 2 completed + 1 failed + 1 running). Sin endpoint nuevo que dispare el mirror desde la PWA — ese refresh privilegiado sigue solo en `MirrorSyncStatusPage`. Frontend: `ProjectFreshnessBanner.tsx` nuevo (usa `Intl.RelativeTimeFormat` porque `t()` de este proyecto no interpola), `types/index.ts` separa `ProjectsApiResponse`/`ProjectsResult`, `formatRelativeTime()` extraído a `utils/relativeTime.ts` (evita romper `react-refresh/only-export-components`). Backend 35/35 tests + Pint limpio; frontend 121/121 tests + `tsc`/`build` limpios + lint sin infracciones nuevas (deuda preexistente del repo no tocada). QA real en vivo (Selenium, NL y EN) confirmó el banner con timestamp real y el ciclo completo del botón refresh (disabled+spinner → normal). `SafetyChecklistSeeder` quedó aplicado en dev (era un gap real del entorno, no del ticket); el `MirrorSyncRun` sintético usado para la captura quedó eliminado. Detalle completo en `CLAUDE.md` sección "CLA-404 (cont.)".

> Última actualización: 2026-08-27 — **CLA-444** (✅ Done): QA real del usuario probando CLA-440 con una foto real (mástil en T + reflector/cámara PTZ montados) encontró dos bugs. (1) La imagen quedaba rota tras "Accepteren en maken" — causa real: `CatalogController::storeGeneratedLuminaireFrameType()`/`storeCustomLuminaireFrameType()` guardaban `image` como URL absoluta (`Storage::disk('public')->url()`, basada en `APP_URL` del backend al crear el registro) en vez de la ruta relativa que usa el catálogo sembrado (resuelta client-side por `resolveApiAssetUrl()`) — esto rompería la imagen igual en producción real (APP_URL interno ≠ dominio público), no era específico del túnel de prueba. Fix: ambos controllers guardan ruta relativa ahora. (2) El prompt de generación seguía agregando placas/herrajes de montaje que no estaban en la foto real — las 6 imágenes de referencia se estaban usando como plantilla de forma, no solo guía de estilo. Prompt reescrito explícitamente para separar ambos roles. Verificado en vivo con la foto real del usuario (recortada del screenshot): resultado sin cámara/reflector, sin herrajes inventados, forma en T fiel a la original. 14/14 tests. Detalle en `CLAUDE.md` sección "CLA-444".

> Última actualización: 2026-08-26 — **CLA-440** (✅ Done): el usuario probó en vivo el resultado de CLA-409 (Gemini 2.5 Flash Image, generación de ilustración de frame type) y lo encontró muy por debajo del estado del arte. POC comparativo real contra `gpt-image-2` de OpenAI confirmó una diferencia de calidad grande (sombreado 3D real, textura metálica creíble vs. resultado plano de Gemini) — reemplazo completo aprobado, sin fallback. Nuevo `OpenAiImageGenerationService` (2 llamadas separadas: `gpt-image-2` para la imagen con transparencia nativa — sin chroma-key, a diferencia de Gemini — y `gpt-5.4-mini` con Structured Outputs para el nombre sugerido, alimentado con la imagen ya generada). Presupuesto de tiempo explícito (imagen 45s/naming 8s) derivado de medir en vivo: el proxy público real (`sbapu03`, `proxy_read_timeout 60s`) hacía que `quality=medium` (56-61s medidos) no tuviera margen real; `quality=low` (26-28s) sí, con calidad igual muy superior a Gemini. Verificado en 2 niveles reales (no mockeados): llamada directa al servicio (27.7s, imagen+nombre reales, alfa real confirmado) y HTTP end-to-end contra los dos endpoints con token Sanctum real (29.9s, `LuminaireFrameType` persistido con transparencia real en disco). 32/32 tests. `GeminiImageGenerationService` borrado; `GoogleServiceAccountAuthService`/config de Gemini-Vertex intactos (siguen en uso por traducción/Watchdog/budget). Detalle en `CLAUDE.md` sección "CLA-440".

> Última actualización: 2026-08-25 — **CLA-439** (✅ Done): a pedido del usuario ("¿cómo se dispararía la sincronización de complejos/clientes/empleados?") se auditó el schedule completo — `intelligence:sync-mirror` (mirror crudo `MirrorRelation`/`MirrorRelationDelivery`) sí corre solo, diario 04:00, pero el bridge FieldOps que lo convierte en `fo_clients`/`fo_complexes` reales (`fieldops:sync-clients-from-relations` FO-012, `fieldops:sync-complexes-from-relation-deliveries` FO-013) nunca estuvo en ningún schedule — solo se corría a mano. Fix: encadenados vía `->onSuccess()` sobre el mismo evento de `intelligence:sync-mirror` en `IntelligenceServiceProvider` (no `->after()`, para no correr contra un mirror que falló a medio camino), en el orden correcto (clients antes que complexes). Verificado invocando el `Event` del schedule directamente contra datos reales de dev (bypassea el chequeo de hora del cron) — un primer intento tiró un error de constraint único que resultó ser un artefacto de dos ejecuciones del propio script de verificación corriendo en paralelo por accidente, no un bug real; confirmado limpio en una segunda corrida (`fo_clients` 1168, `fo_complexes` 893, orden de `updated_at` correcto). 21/21 tests en el filtro `SyncClientsFromRelations|SyncComplexesFromRelationDeliveries|MirrorSync`. Detalle en `CLAUDE.md` sección "CLA-439".

> Última actualización: 2026-08-25 — **CLA-404** (🚧 parcial, no cerrado en Linear): sync manual del mirror Intelligence, que vivía enterrado en `BiConfigPage` (solo `super_admin`, sin indicador de última corrida ni guard contra doble-disparo), ahora en página dedicada `Mirror Sync Status` (Intelligence Hub), accesible también a `admin`/`financial_manager`. Tabla `intelligence_mirror_sync_runs` + guard atómico (mismo patrón que `SyncDashboardPage` de Prospects). **Pendiente:** el banner de frescura en la vista de proyectos de Safety Inspections (punto 4 del alcance original) — ticket queda `In Progress`. Detalle en `CLAUDE.md` sección "CLA-404".

> Última actualización: 2026-08-24 — **CLA-409 / CLA-390 Fase 3** (✅ Done, `6b1551c` backend + `1105d5d` en `Claesen-Sport-updateing`): última fase del plan de 3 de CLA-390 — cuando la Fase 1 (match contra catálogo) no encuentra nada, se genera una ilustración nueva del frame en el estilo visual del catálogo (`public/assets/frame-types/*.png`) con fondo transparente + nombre sugerido por IA editable, en vez de limpiar el fondo de la foto real (pedido explícito del usuario). `GeminiImageGenerationService` (nuevo, `gemini-2.5-flash-image` vía Vertex AI/CLA-407) downscalea las 6 imágenes de referencia + la foto antes de enviar — un payload sin downscale (~1.4MB) era interceptado por una página de bloqueo anti-abuso de Google antes de llegar a la API real. Gemini no genera transparencia real pese a afirmarlo (verificado leyendo los bytes del PNG) — se le pide un fondo sólido y se convierte a alfa real con GD del lado del servidor. Endpoint de solo lectura para la propuesta + endpoint de persistencia (`source=ai_generated`/`verified_by_user_id=null`, mismo patrón de gobernanza que CLA-389). **Bug real encontrado y corregido en la verificación en vivo (no la teoría de `<StrictMode>` sospechada antes, que resultó ser pista falsa):** el efecto de generación en `CameraCapture.tsx` depende de `matching`/`matchResult` además de `capturedImage` — se re-invocaba a mitad de vida de la misma foto, y el `cancelled` de closure cancelaba en silencio la respuesta ya exitosa (confirmada completa vía Performance API, sin ningún error en consola), dejando el spinner colgado para siempre. Fix: guard basado en ref ("¿sigue siendo esta la foto actual?") en vez de cancelación por cambio de dependencia. Backend 28/28 tests, frontend `tsc`/lint/vitest limpios. **Verificado en vivo con Selenium contra un build de producción real** (no dev server): foto → sin match → generación automática → propuesta con nombre editable → aceptar → `LuminaireFrameType` real persistido con fondo transparente confirmado por canal alfa real en disco. Detalle completo en `CLAUDE.md` sección "CLA-409".

> Última actualización: 2026-08-21 — **CLA-407** (✅ Done, `f0d288b`): disparado por el POC de Fase 3 de CLA-390 (generación de imagen) — `GEMINI_API_KEY` (misma key que `GOOGLE_GEOCODING_API_KEY`) devolvía 403 en cualquier llamada a Gemini, sin importar el método. Diagnóstico completo contra Google Cloud Console real: la key solo permite "Geocoding API"; no se puede agregar Gemini a esa key ni crear una key nueva con Gemini (política de organización exige cuenta de servicio); y la propia Generative Language API rechaza tokens OAuth2 de cuenta de servicio ("Use authorization keys instead"). **Fix real: migrar a Vertex AI** (mismo Gemini, endpoint `aiplatform.googleapis.com`/"Agent Platform", que sí soporta OAuth2/SA nativo) — nueva cuenta de servicio acotada (`gemini-vision-service@...`, rol `Vertex AI User` únicamente, no reutilizada la cuenta `Owner` existente sin usar). Nuevo `GoogleServiceAccountAuthService` (JWT RS256 + intercambio OAuth2, cacheado) usado por los 6 lugares del código que llaman a Gemini (`GeminiService` + 5 más encontrados por grep: `AuditProjectService`, `TechnicianAnalysisService`, `GenerateBudgetSimulationJob`, `MaterialIntelligenceService`, `BudgetAssistantService`) — ninguno pasaba antes por `GeminiService`. Bug real de Vertex encontrado en vivo: exige `role: 'user'` explícito en `contents[]` (la API vieja lo toleraba omitido), corregido en los 6. Regresión real de un test (`TechnicianAnalysisServiceLocaleTest`, subclase anónima sin constructor args) corregida. Validado con llamadas reales (traducción ES→NL/EN/FR/DE correcta, análisis financiero real coherente) + 37/37 tests afectados sin regresiones. **Pendiente fuera de este cierre:** confirmar si la key de producción tiene la misma restricción (no verificable desde este entorno, sin SSH directo a `192.168.254.52`) — si la tiene, la traducción automática real está rota en prod ahora mismo. Detalle en `CLAUDE.md` sección "CLA-407".

> Última actualización: 2026-08-21 — **CLA-408** (✅ Done, `5877a1b`): reportado por el usuario probando `/fo-maintenance-plans` — creó una work order desde "Schedule maintenance" esperando que generara un `FoMaintenancePlan` recurrente, pero la tabla `fo_maintenance_plans` seguía vacía (verificado con `withTrashed()`, 0 filas en dev). No era un bug de listado: `MaintenanceWorkOrderService::create()` solo genera el plan si `recurrence_unit` no está vacío, y el usuario había dejado ese select sin elegir — el campo `recurrence_interval` sí mostraba `1` (su `->default(1)`), dando la falsa impresión de que la recurrencia ya estaba configurada. Fix: se quita el default engañoso de `recurrence_interval` y se agrega validación cruzada (`requiredWith()` en ambas direcciones, `FoMaintenanceWorkOrderResource::form()`) — dejar solo uno de los dos campos completado ahora falla con un mensaje claro en vez de crear la work order sin plan en silencio. 4 tests nuevos, 17/17 en `MaintenanceWorkOrderFilamentTest`, 69/69 en regresión amplia (`MaintenanceWorkOrder|MaintenancePlan|MaintenanceRequest`). Detalle en `CLAUDE.md` sección "CLA-408".

> Última actualización: 2026-08-20 — **CLA-405** (✅ Done, `d546ba0`): reportado por el usuario (`orelvys.cuellar@claesen-verlichting.be`) probando `service.claesen-verlichting.be` en producción como `super_admin` — la página "Rapporten" (Reports) mostraba "Unable to load work order history". Diagnóstico confirmado con evidencia real de nginx access log en prod (múltiples 404 en `GET /api/v1/fieldops/maintenance-work-orders/history`, `referer: .../app/reports`): la ruta **nunca se implementó** en el backend; `/maintenance-work-orders/{workOrder}` (registrada justo después de `/assigned`) capturaba `"history"` como ID vía route-model-binding implícito y devolvía 404. No era específico de rol — le pasa a cualquier usuario. Fix: `MaintenanceWorkOrderController::history()` (calcado de `assigned()`, filtrado a `completed`/`cancelled`, mismo scope super_admin/admin=todo vs. resto=solo lo propio) + ruta registrada antes del wildcard. 3 tests nuevos, 21/21 en `MaintenanceWorkOrderTest`, 85/85 en regresión ampliada. Diagnóstico ejecutado delegando a una instancia de Claude Code corriendo en `prod-priv-01` (solo lectura, sin tocar nada) — confirmó también que el rol `super_admin` del usuario está correcto y limpio en BD de producción (sin duplicados de cuenta), así que el problema paralelo de "no veo Complexes/Clients en el menú" **no es de datos** → ver CLA-406.
>
> **CLA-406** (✅ Done, 2026-08-21): el diagnóstico inicial (20/8) creyó que el pipeline de deploy de `service.claesen-verlichting.be` era desconocido/estaba roto, comparando contra el runner equivocado (`safety_claesen`, otro proyecto). **Corregido al día siguiente:** la causa real era que el checkout local (`Claesen-Sport-updateing`, rama `fieldops-port`) tenía 4 commits (`bc3b389` y los 3 anteriores, CLA-388→391) comiteados pero **nunca pusheados a GitHub** (`git status -sb` → `ahead 4`) — el pipeline real (`.github/workflows/deploy.yml`, rama default `deploy`, runner en `/home/bert/actions-runner-sport/` en sbapu03) funcionaba bien, nunca tuvo la chance de construir algo que no había llegado al remoto. Fix ejecutado sin tocar infraestructura: `git push origin fieldops-port` + fast-forward de `fieldops-port` sobre `deploy` desde un worktree temporal aislado (para no pisar una sesión paralela con cambios sin comitear) + push a `deploy`. Verificado con evidencia real: `gh run list` → run del commit `bc3b389` completed/success (run id `32467022962`); `curl -I https://service.claesen-verlichting.be/` → `Last-Modified` saltó a 2026-08-21 09:17:57, hash de bundle cambiado. Los 3 webroots (`service`/`safety`/`fieldops.claesen-verlichting.be`) son 3 apps separadas por diseño, no un problema de sprawl. Detalle completo en `CLAUDE.md` sección "CLA-405 / CLA-406".

> Última actualización: 2026-08-19 — **CLA-391 / CLA-390 Fase 2** (✅ Done, `3681578` backend + `bc3b389` en `Claesen-Sport-updateing`): al fotografiar un frame con luminarias montadas, la IA detecta cada fixture visible con posición aproximada (x/y normalizados) e intenta identificar su tipo (`ClaudeVisionService::detectLuminairesInFrame()`, método hermano de Fase 1). Sin match → `PlaceholderLuminaireTypeSeeder` (nuevo, seed idempotente en `FieldOpsCatalogSeeder`/`RebuildLuminaireCatalogSeeder`) asigna un tipo "Unidentified fixture — pending replacement", reemplazable después vía el flujo existente "Replace luminaire" (CLA-265). Sin bulk endpoint nuevo — el frontend confirma detecciones llamando secuencialmente al `POST /luminaires` existente, una vez por detección aceptada. Nuevo `DetectLuminairesModal.tsx` (patrón `<input type="file" capture>`, mismo que `VisionIdentifyPanel.tsx`) con lista de confirmación por checkbox; sin canvas de arrastre nuevo, la posición se ajusta después en el canvas ya existente. **Verificado en vivo con Selenium** contra el frame real de demo: foto vacía → "ninguna detectada" correcto; foto real de producto BVP518 → detección real con evidencia, sin match de catálogo → placeholder creado con posición exacta, contador del canvas actualizado en vivo, toast de éxito. 24/24 tests backend nuevos/afectados + regresión amplia 172/174 (2 fallos preexistentes no relacionados, confirmados contra HEAD). 4/4 tests frontend nuevos, `tsc`/lint limpios. Fase 3 (generación de imagen) sigue sin ticket. Detalle completo en `CLAUDE.md` sección "CLA-391".

> Última actualización: 2026-08-19 — **CLA-390 Fase 1** (✅ Done, `11890a0` backend + `10ac80f` en `Claesen-Sport-updateing`): al fotografiar un frame para crearlo, primero se intenta identificar el tipo contra el catálogo interno de `LuminaireFrameType` (nuevo endpoint `POST /api/v1/fieldops/luminaire-frame-types/vision-suggestions` + `ClaudeVisionService::identifyFrameType()`, método hermano de `identifyLuminaires()`) — un match confiable deja confirmar el tipo existente en un tap; sin match (o al descartar) cae al flujo manual "crear frame personalizado" sin cambios. Alcance explícitamente acotado a Fase 1 de 3 (multi-luminaria + generación de imagen quedan fuera, sin ticket todavía, "no crear tickets hasta cerrar esta fase"). 12/12 tests backend nuevos + 4/4 frontend (`cameraCaptureUtils.ts` extraído para no romper fast-refresh, mismo patrón que `visionSuggestions.ts` de CLA-389). **Verificado en vivo con Selenium usando `--use-fake-device-for-media-stream`** (Chrome headless sin cámara real): modal de cámara real, captura real, llamada real a Claude Sonnet 5 (sin errores en logs), y caída correcta al formulario manual cuando el patrón sintético de Chrome no matchea nada — la rama "match encontrado" (tarjetas Sparkles) quedó verificada por lectura de código + tests, no por render en vivo (no había forma de inyectar una foto real de un frame conocido a través de una cámara falsa). Estructura QA temporal creada/borrada durante la verificación. Detalle completo en `CLAUDE.md` sección "CLA-390".

> Última actualización: 2026-08-19 — **CLA-389** (✅ Done, `1b83d63` backend + `6e240e4` en `Claesen-Sport-updateing`): identificación visual ampliada a marca+modelo fuera del catálogo interno (Philips/Signify, Schréder, Thorn, Musco), no solo los 10 `LuminaireType` sembrados. Gobernanza elegida por el usuario: creación inmediata + flag `source=ai_suggestion`/`verified_by_user_id=null` (no cola bloqueante) — backoffice revisa después desde Filament (Catalogs → Luminaire Types/Subgroups, badge "Sin verificar" + acción "Marcar como verificado"). Guía de usuario completa (técnico + backoffice) en `docs/luminaire-vision-identification-user-guide.md`. 17 tests nuevos backend (159/159 en regresión ampliada) + 1 test nuevo frontend (34/36, mismos 2 fallos preexistentes de siempre). **Verificación en vivo honesta:** 5 llamadas reales a Claude Sonnet 5 con fotos reales fuera de catálogo nunca dispararon el camino de sugerencia externa — el modelo siempre encontró un match interno plausible o abstuvo genuinamente por falta de logo visible; el camino de código está verificado por tests + por el schema/prompt confirmados correctos en esas 5 llamadas, pero no se vio el render en vivo con una sugerencia real. **Incidente de sesión:** un script de debug vía `artisan tinker` contaminó la DB real de desarrollo con 2 usuarios/1 subgroup/1 type falsos (Faker) — detectado y limpiado a mano, memoria `feedback_tinker_factory_pollution` creada. Detalle completo en `CLAUDE.md` sección "CLA-389".

> Última actualización: 2026-08-19 — **CLA-388** (✅ Done, `5e7e0f7` en `Claesen-Sport-updateing`) + **CLA-389** (nuevo, Todo): CLA-388 conectó el flujo de identificación por foto + tab Media al frontend real. **Incidente de sesión, ya corregido:** el trabajo se construyó primero contra `/home/totti/Claesen-Sport` (repo local viejo, sin remoto, último commit 2026-08-08) hasta que una captura del usuario reveló que el frontend real es `/home/totti/Claesen-Sport-updateing` (remoto `service.claesen-verlichting.git`, CI de deploy a producción) — se rehizo ahí, verificado en vivo con Selenium (sesión real, click-through real, foto real, candidatos reales BVP518/BVP528 con evidencia e imagen correctas, tab Media funcionando). El usuario probó con 3 fotos reales de Philips/Signify/Schréder/Thorn/Musco — las 3 dieron "unknown" porque esos productos no están en los 10 `LuminaireType` internos (confirmado por el usuario, no un bug). Su objetivo real es buscar contra el catálogo completo de esas marcas → **CLA-389** creado (Todo, sin plan todavía) con dos diseños evaluados: ampliar el catálogo a mano (descartado) vs. sugerencia de marca/modelo en texto libre no autoritativa + confirmación humana (elegido, pendiente de plan detallado). Detalle completo en `CLAUDE.md` secciones "CLA-388"/"CLA-389".

> Última actualización: 2026-08-18 — **CLA-386** (✅ Done, `417801c`): cerrado tras validación end-to-end (ver detalle en la entrada del 2026-08-18 más abajo) y decisión explícita del usuario de aceptar el alcance backend-only. Auditoría de código sobre `/home/totti/Claesen-Sport` confirmó que ni el flujo de foto+identificación ni un catálogo "ampliado por consulta a marcas" existen en el frontend — el selector manual sigue siendo dos `<select>` en cascada sin cambios desde el 2026-07-05. Follow-up creado en Linear: **CLA-388** ("conectar identificación visual — foto + confirmación — en Claesen-Sport"), estado Todo, sin iniciar, con plan pendiente de aprobación antes de tocar código. Detalle completo en `CLAUDE.md` sección "CLA-386".

> Última actualización: 2026-08-17 — **CLA-387** (In Progress): investigación de modelos IA vigentes (agosto 2026), disparada por una pregunta del usuario sobre si Gemini 1.5 Flash (documentado en `CLAUDE.md`) estaba desactualizado. Hallazgo real: `.env`/`.env.remote` ya usaban el alias rotativo `gemini-flash-latest` — el riesgo real no era el modelo en producción sino el *fallback* hardcodeado a `gemini-1.5-flash(-latest)` en 4 archivos, encontrados por grep de todo el repo (no de memoria): `Modules/Intelligence/Services/GeminiService.php`, `Modules/Intelligence/Jobs/GenerateBudgetSimulationJob.php`, `Modules/Performance/Services/AuditProjectService.php` (Watchdog) y `Modules/Performance/Services/TechnicianAnalysisService.php` (arquetipos de técnico) — solo se ejecuta si falta la config, pero es un valor por defecto peligroso. Los 4 se alinearon a `gemini-flash-latest`; un texto de UI hardcodeado en `project-intelligence-detail.blade.php` también mencionaba "Gemini 1.5 Flash" y se corrigió. `CLAUDE.md`/`docs/ai/context-map.md` actualizados. Sin Docker en esta sesión (`sail ps` → "Docker is not running") para correr la suite completa; se verificó `php -l` en los 5 archivos PHP tocados.

> Última actualización: 2026-08-18 — **CLA-386** (In Progress, validado end-to-end): de la misma investigación de modelos IA salió la decisión de usar **Claude Sonnet 5** (no Gemini) para un MVP de identificación visual de luminarias en frames FieldOps — se acotó una propuesta muy amplia (microservicio Python, MCPs propios, embeddings visuales, reconstrucción 3D) a lo mínimo viable. Nuevo `Modules/Intelligence/Services/ClaudeVisionService.php` vía el SDK oficial `anthropic-ai/sdk` (Composer, `v0.42.0`), con `output_config.format` (JSON Schema estricto) pidiendo candidatos con `confidence`/`evidence`/`status: identified|probable|unknown` — "no identificado" es una respuesta válida y preferida, nunca se fuerza una identificación. Endpoint nuevo `POST /api/v1/fieldops/luminaire-frames/{frame}/vision-suggestions` (`LuminaireVisionController`, solo lectura, nunca crea/edita el frame).
>
> **Sesión 2026-08-18 — validación con Docker + llamada real:** `sail up -d` levantado, `LuminaireVisionControllerTest` (4/4) y `ClaudeVisionServiceTest` (2/2) en verde, 6/6 — 15 assertions, todos mockeados (sin red). Además se corrió una llamada real end-to-end contra `ClaudeVisionService` (vía `artisan tinker`, no vía HTTP) usando la API key real ya cargada en `.env` y el catálogo real sembrado en dev (10 `LuminaireType`), con la foto oficial del producto BVP518 (`public/assets/luminaire-types/bvp518-optivision-led-gen3-5.png`) como input. Primer intento falló con crédito insuficiente en la cuenta de Anthropic (`Claude Vision Exception: ... credit balance is too low`, capturado en logs, silenciosamente devuelto como `unknown`/`[]` por el catch genérico del servicio — comportamiento correcto pero indistinguible en el JSON de una abstención real por falta de evidencia visual). Tras cargar $5 de crédito en console.anthropic.com, la llamada real funcionó: `status: probable`, candidato top `catalog_id: 27` (BVP518, el correcto) con confianza 0.45 y evidencia visual concreta (housing de doble módulo, bracket/trunnion, acabado y aletas de disipación), más `catalog_id: 28` (BVP528, familia hermana) como alternativa a 0.35 — nunca forzó una identificación con certeza no soportada, exactamente el comportamiento de diseño. **No se probó por la ruta HTTP real (Sanctum auth) en esta sesión**, solo el servicio directo — el endpoint completo ya está cubierto por los tests mockeados. Pendiente: decisión del usuario de cerrar el ticket y preparar la demo para gerencia (pedir presupuesto de API).

> Última actualización: 2026-08-14 — **CLA-380** (Done): follow-up de CLA-379 — el usuario preguntó si había un skill/mecanismo que garantizara leer `DESIGN.md` antes de tocar UI. No lo había: solo quedó una mención pasiva en `CLAUDE.md`, sin enforcement real. Se agregó una sección "UI / Visual identity" en `.agents/rules/00-project-startup.md` (el archivo "Always On" leído antes de cualquier edición) y la regla 11 en la lista no-negociable de `CLAUDE.md`: leer `DESIGN.md` antes de crear/modificar Blade/Filament/tokens Tailwind, correr `npx @google/design.md lint DESIGN.md` si se tocan colores/tipografía, y reportar/corregir si diverge del código real. No se tocó `DESIGN.md` ni ningún componente de UI. Commit `db38ead` (rama `fieldops-backend-fixes`).

> Última actualización: 2026-08-14 — **CLA-379** (Done): el usuario pidió analizar el `design.md` que Google acaba de sacar para Stitch y "conectarse a su MCP". Aclarado que no es un MCP — es una especificación de formato + CLI local (`@google/design.md`, sin auth), no un servicio al que conectarse. Se creó `DESIGN.md` en la raíz documentando la identidad visual real del backoffice (paleta Claesen Cyan/Lime/Magenta/Amber + acento Orange "signature" de `AdminPanelProvider.php`/`tailwind.config.js`/`app.css`, tipografía Outfit, excepción de PDFs), sin tocar ningún archivo de producción. `npx @google/design.md lint DESIGN.md`: 0 errores; quedan 3 warnings de contraste WCAG AA documentados en el propio archivo como hallazgo conocido (corregir los colores reales es una decisión de producto aparte). Commit `0f8870b` (rama `fieldops-backend-fixes`). Detalle en `CLAUDE.md` sección "Identidad visual — DESIGN.md".

> Última actualización: 2026-08-13 — **CLA-378** (Done): verificación pre-push (corrida completa de FieldOps/Core antes de reconciliar `fieldops-backend-fixes` → `main`) encontró 5 fallas reales preexistentes en `FieldOpsMediaTest`/`MaintenanceRecordCrudTest` — deuda de CLA-369 (usuarios de test sin rol, fail-closed tras el scoping tenant-aware de registro único). No era regresión de comportamiento real. Fix: mismo tratamiento que CLA-369 — los helpers compartidos otorgan `fieldops.view-all-clients`. 27/27 en verde. Detalle en `CLAUDE.md` sección "CLA-378".

> Última actualización: 2026-08-13 — **CLA-377** (In Progress → cierra con este commit): decisión de negocio explícita del usuario, sin ticket previo — `project_manager` recupera `fieldops.view-all-clients` en `RolesAndPermissionsSeeder` (revierte parcialmente CLA-364; `technician` se queda scoped, sin cambios). Test existente que asumía lo contrario (`test_project_manager_without_the_broad_permission_is_also_scoped`) sigue en verde porque construye el rol de forma aislada en su `setUp()`, sin correr el seeder real — comentario actualizado para no inducir a error sobre el comportamiento real en producción. Backfill pendiente en prod: `php artisan db:seed --class=RolesAndPermissionsSeeder`. 54/54 en verde en el filtro amplio de FieldOps tenant/work orders. Detalle en `CLAUDE.md` sección "CLA-377".

> Última actualización: 2026-08-13 — **CLA-376** (Done): `GET /fo-maintenance-work-orders/18` tiraba 500 — el `formatStateUsing()` del `TextEntry` de `completion_details` (agregado en CLA-374) asumía recibir el array completo, pero Filament lo fragmenta valor-por-valor cuando `count($state) > 1`. Fix: `->state(fn ($record) => ...)` leyendo `$record->completion_details` directo. 1 test nuevo, 13/13 en verde en `MaintenanceWorkOrderFilamentTest`. Detalle en `CLAUDE.md` sección "CLA-376".

> Última actualización: 2026-08-10 — **CLA-375** (Done): reportado por el usuario como un 403 real en el frontend (`GET /fieldops/complexes/27`) al abrir el detalle de una tarea asignada desde "My Tasks". Causa: la asignación de una work order (CLA-271) nunca otorga scope en `fieldOpsClients` (CLA-364) — un técnico veía su tarea listada (`assigned()` filtra solo por `assigned_employee_id`) pero `FieldOpsTenantService::canView()` lo bloqueaba al abrirla o al ver el equipo vinculado, porque exigía que el cliente estuviera en `allowedClientIds()`. Fix: `canView()` también considera los clientes derivados de las work orders asignadas al empleado (`assignedWorkOrderClientIds()`), sin tocar `scopeForUser()` (listados). Un test preexistente (`test_technician_cannot_view_a_work_order_for_another_client`, CLA-369) fijaba este mismo bug como comportamiento esperado — corregido para seguir probando aislamiento real sin contradecir el fix. 2 tests nuevos. 14/14 en verde en `FieldOpsTenantAuthorizationTest`; 87/88 en el filtro amplio (1 fallo preexistente no relacionado, confirmado contra HEAD). Detalle en `CLAUDE.md` sección "CLA-375".

> Última actualización: 2026-08-10 — **CLA-374** (Done): pedido explícito del usuario tras probar el flujo completo en `Claesen-Sport-updateing` — el Edit de una work order en Filament no exponía `root_cause`/`solution_applied`/`completion_notes`/`completion_details` para revisar/corregir el reporte de campo antes de validar. Nuevo `MaintenanceWorkOrderService::updateReview()` (solo sobre `awaiting_validation`, whitelist + evento `reviewed`); sección "Review" + sección "Tasks performed" con checkboxes (no texto libre) en vez de reescribir todo a mano; planificación se bloquea en ese estado. Infolist ahora muestra `completion_details` legible. 4 tests nuevos, 36/36 en verde. Detalle en `CLAUDE.md` sección "CLA-374".

> Última actualización: 2026-08-08 — **CLA-301** (Done, repo `Claesen-Sport-updateing`): la rama `fieldops-port` (frontend) nunca se había pusheado a `origin` — quedó respaldada (`git push -u origin fieldops-port`, ya existe `origin/fieldops-port`). Working tree cerrado en 2 commits: `4f84885` (agrega `typescript-eslint` a `package.json`, faltaba y rompía `npm run lint` con `ERR_MODULE_NOT_FOUND` porque `eslint.config.js` lo importa sin tenerlo declarado como dependencia) y `bc87ceb` (HANDOFF.md/README.md/`.claude/settings.local.json`). `npm install` requirió `--legacy-peer-deps` por un conflicto preexistente no relacionado (`vite@^7.0.5` vs `@vitejs/plugin-basic-ssl` que pide `^6.0.0`). Lint ya corre, pero reporta 405 errores/52 warnings preexistentes en todo el repo — deuda técnica documentada, no tocada (fuera de alcance, "restaurar el comando" no es "arreglar el código"). **Gotcha real:** `npm install` en este repo modifica `yarn.lock` como efecto colateral aunque el proyecto use npm (`package-lock.json` es el lockfile real) — se descartó (`git checkout -- yarn.lock`), no comitear ese archivo sin revisar antes. Dejado explícitamente sin tocar, decisión pendiente del usuario: `dist/` (86 archivos de build borrados sin reemplazo, directorio trackeado pese a estar en `.gitignore`, viene de la rama `deploy-dist` — ambiguo/riesgoso comitear sin entender el flujo de deploy real).

> Última actualización: 2026-08-07 — **CLA-362** (Done): la sesión anterior se cortó por un cierre inesperado de la máquina (contenedores de Sail caídos, diff sin comitear) mientras extendía CLA-351 más allá de lo que su cierre documenta. CLA-351 solo expuso `structure_id`/`terrain_id`/`complex_id` en `equipment` de `MaintenanceWorkOrderResource`, pero su descripción original ya pedía los 6 segmentos de URL (`complex/terrain/structure/lat/lng/frame+posición`) para el enlace de vuelta desde "My Tasks" al detalle de la luminaria. CLA-362 retoma y cierra ese trabajo: `luminaire_frame_id` + `lat`/`lng` del `Terrain` ya resuelto, mismo alcance (solo `Luminaire`, `null` para `ElectricalBoard`) y mismas queries (sin extra). De paso, `config/cors.php` suma `http://localhost:5180`. Test existente (`test_assigned_queue_only_returns_the_workers_orders_with_equipment_context`) ganó 6 aserciones nuevas verificando los 6 campos contra datos reales del test. 26/26 en verde (126 assertions). Commit `ddf16fc` (rama `fieldops-backend-fixes`). Detalle en `CLAUDE.md` sección "CLA-362".

> Última actualización: 2026-08-04 — **CLA-340** (Done): reportado por el usuario, "cambio la localización de complejo después de actualizar la posición del pin no se mantiene una vez que se modifica". Mismo bug que CLA-305 corrigió para `Terrain` (commit `bc29b00`, 2026-08-03), nunca portado a `Complex`/`Structure`/`ElectricalBoard`: sin `mutateFormDataBeforeFill()` sincronizando `map_center_lat/lng = lat/lng` ni `afterSave()` (`refresh()`+`fillForm()`), el form recaía en las coordenadas viejas al re-llenarse tras guardar; y los pickers Alpine no pasaban `syncCenterFields=true` en `dragend`/`click` ni disparaban `dispatchEvent` al escribir los inputs "centro". Portado el mismo diff mecánico a los 3 recursos. 3 tests de persistencia nuevos (arrastrar→guardar→verificar). **Hallazgo colateral:** el `assertSee(..., false)` del test original de Terrain (CLA-305) no puede pasar nunca — el contenido de un bloque `@script` de Livewire viaja HTML-escapado dentro del atributo `wire:effects`, no como texto plano; corregido en los 3 tests nuevos con `assertSee($needle)` (escape por defecto), mismo patrón ya documentado para breadcrumbs (CLA-278 cont.8). El test de Terrain quedó sin tocar — falla por una causa previa no relacionada (`defaultPinVariant`), confirmada como preexistente en HEAD antes de este ticket, fuera de alcance. Commit `128ddc3` (rama `fieldops-backend-fixes`). Detalle en `CLAUDE.md` sección "CLA-340". reportado por el usuario, "cambio la localización de complejo después de actualizar la posición del pin no se mantiene una vez que se modifica". Mismo bug que CLA-305 corrigió para `Terrain` (commit `bc29b00`, 2026-08-03), nunca portado a `Complex`/`Structure`/`ElectricalBoard`: sin `mutateFormDataBeforeFill()` sincronizando `map_center_lat/lng = lat/lng` ni `afterSave()` (`refresh()`+`fillForm()`), el form recaía en las coordenadas viejas al re-llenarse tras guardar; y los pickers Alpine no pasaban `syncCenterFields=true` en `dragend`/`click` ni disparaban `dispatchEvent` al escribir los inputs "centro". Portado el mismo diff mecánico a los 3 recursos. 3 tests de persistencia nuevos (arrastrar→guardar→verificar). **Hallazgo colateral:** el `assertSee(..., false)` del test original de Terrain (CLA-305) no puede pasar nunca — el contenido de un bloque `@script` de Livewire viaja HTML-escapado dentro del atributo `wire:effects`, no como texto plano; corregido en los 3 tests nuevos con `assertSee($needle)` (escape por defecto), mismo patrón ya documentado para breadcrumbs (CLA-278 cont.8). El test de Terrain quedó sin tocar — falla por una causa previa no relacionada (`defaultPinVariant`), confirmada como preexistente en HEAD antes de este ticket, fuera de alcance. Commit `<pendiente>` (rama `fieldops-backend-fixes`). Detalle en `CLAUDE.md` sección "CLA-340".

> Última actualización: 2026-08-04 — **CLA-339** (Done): TypeError reportado por el usuario al editar/asignar una Maintenance Work Order (`assertAssignableEmployee(): Argument #1 must be of type ?string, int given`). Causa: `employees.id` es string, pero PHP castea a `int` cualquier key de array numérica-string (ej. `"100"`), y el `Select` de Filament usa el id como key de sus `options()` — el `int` resultante rompía `MaintenanceWorkOrderService` (`declare(strict_types=1)`). Intentar arreglarlo casteando `(string)` al construir el array de opciones no funciona (la coerción de PHP es sobre la key en sí, no revertible con un cast en userland); el fix real normaliza `assigned_employee_id` a `?string` dentro de `MaintenanceWorkOrderService::create()`/`updatePlanning()`, en el punto donde se lee de `$data`. Commit `80db935` (rama `fieldops-backend-fixes`). Detalle en `CLAUDE.md` sección "CLA-339 — fix TypeError en asignación de Maintenance Work Orders".

> Última actualización: 2026-08-04 — **CLA-295**: `main` local estaba 191 commits por detrás de `origin/main` (sin push desde 2026-07-07) y `fieldops-backend-fixes` (803 commits, ~6 semanas de trabajo FieldOps, CLA-228→CLA-333) nunca se había subido al remoto. Reconciliado en un worktree aislado (`/tmp/claesen-api-main-integration`, ya existente de CLA-329): cherry-pick del único commit exclusivo de `main` (`921098f`, documental) hacia `fieldops-backend-fixes`, luego merge limpio (sin conflictos) de `fieldops-backend-fixes` en `main` → commit `549dd2c`, 805 commits totales. `origin/main` sigue siendo ancestro del `main` local, así que el push pendiente sería normal (no forzado). **Push a `origin/main` NO ejecutado todavía** — pendiente de decisión explícita del usuario. Documentada la regla de rama fuente de verdad en `CLAUDE.md` (sección FieldOps). De paso: se encontró un Personal Access Token de GitHub en texto plano en la URL del remote `origin` (`git remote -v`) — pendiente de rotación por el usuario, no es código, no se tocó.
>
> Contexto del incidente que motivó retomar CLA-295 con más cuidado: a mitad de esa misma investigación, un `git reset --hard HEAD` mal usado como fallback descartó ediciones sin comitear de otro ticket (CLA-333, In Progress). Se reconstruyó desde el ticket de Linear (no restauración byte-exacta) y se comiteó como `8ddbcf6` — ver comentario en CLA-333 para el detalle. Lección aplicada desde entonces: nunca `reset --hard`/`checkout --`/similar como salida rápida de un fallo de git sin revisar `git status` y confirmar que no hay cambios sin comitear en juego.

> Última actualización: 2026-08-03 — **CLA-329** integró `fieldops-backend-fixes` en `main` mediante fast-forward, llevando el commit `35a64ec` de **CLA-304**. CLA-304 permite al agente crear y mover automáticamente los tickets Linear necesarios para solicitudes autorizadas con cambios, manteniendo plan, aprobación antes de editar, pruebas/checks, GO técnico y cierre controlado. **WAIVER — CLA-304:** cambio exclusivamente documental; riesgo residual = incoherencia entre reglas; cobertura alternativa = revisión cruzada de las reglas y `git diff --check`; aprobado por GO técnico del usuario el 2026-08-03.

> Estado global vivo del proyecto. Actualizar en cada cierre de ticket.
> Última actualización: 2026-08-02 — **CLA-278 cerrado en Linear** (Done, confirmado por el usuario en navegador real). Como continuación sin ticket formal (Linear bloqueado por `usage limit exceeded`, usuario aprobó proceder igual), se extendió el sistema de breadcrumbs jerárquicos: páginas **Create** de Terrain/Structure/Electrical Board/Luminaire (`e868110`) + Luminaire Frame, olvidado en la primera pasada (`2dc5936`) + **todas las páginas de Electrical Board** con propagación de contexto de punta a punta (`efcb586`) + **ocultarlo del sidebar/breadcrumb + fix de un título de breadcrumb vacío real** (`b36807f`) + **fix del redirect post-create que perdía el contexto `via_*` en las 4 páginas Create** (`e2f71a5`, causa real: `request()` no es confiable dentro de una invocación `->call()` de Livewire, hay que capturar el contexto en una propiedad durante `mount()`) + **contexto de mapa para Electrical Board** (pines y "Map objects" ya conservan `via_complex`/`via_terrain`/`via_structure`) + **Attach visible y funcional desde Structure** + **picker de Electrical Board adaptado a light/dark**. Detalle en `CLAUDE.md` sección "FieldOps: breadcrumbs jerárquicos en páginas Create...".
...
- Se portó el set de estilos de `terrain-location-picker.blade.php` usando los selectores propios de Electrical Board y se retiró el atributo de tema estático, dejando que el `init()` existente lo asigne dinámicamente.
- El registro Alpine `@push('scripts')`/`@once` no se tocó: es el riesgo ya documentado para los cuatro location-pickers y no se amplió el alcance a ese ticket separado.
- **Validación:** Playwright abrió Create Electrical Board y verificó estilos calculados en ambos temas: dark → `data-theme="dark"`, gradiente `#171725 → #11111a`, título claro y footer `#94a3b8`; light → `data-theme="light"`, gradiente blanco, título `#0f172a` y footer `#64748b`. Usuario confirmó visualmente. Sin test PHPUnit aplicable para este ajuste Blade/CSS; la evidencia Playwright es la cobertura de UI.

### Sesión 2026-08-02 — FieldOps: Attach Electrical Board desde Structure (sin ticket formal)

**Contexto:** el usuario reportó que la pestaña “Electrical boards” de `/structures/{id}?relation=2` solo mostraba “Create electrical board”; faltaba el control para adjuntar un board existente. Playwright confirmó el defecto inicial (`attachCount: 0`) en la pestaña realmente montada, no solo por lectura de PHP.

- Se igualó primero `AttachAction` a Complex/Terrain (label, button, color, selector multilingüe y `syncWithoutDetaching()`), pero Filament seguía ocultando esa acción incluso con `->visible(true)`. Se sustituyó únicamente en Structure por `Action::make('attach')` con la misma interfaz/modal y persistencia explícita; no depende del guard interno que escondía `AttachAction`.
- Playwright confirmó el resultado real: botón “Elektrisch bord koppelen” visible (`attachCount: 1`), modal con selector y opciones, selección de `#9`, submit y relación persistida `[7,9]`. El pivot temporal se desadjuntó después y se eliminó el usuario QA temporal.
- Se añadió `ElectricalBoardFilamentTest::test_structure_electrical_boards_relation_manager_attaches_an_existing_board()`. Sintaxis PHP y `git diff --check` pasan. **WAIVER aprobado explícitamente por el usuario con GO técnico:** PHPUnit quedó bloqueado por el harness `testing` antes de concluir; cobertura alternativa = Playwright E2E del flujo exacto + test de regresión añadido. No mezclar la reparación de ese harness con este fix.

### Sesión 2026-08-02 — FieldOps: enlaces de Electrical Board desde el mapa conservan el breadcrumb (sin ticket formal)

**Contexto:** el usuario reportó que al abrir `/electrical-boards/1` desde un pin del mapa o desde “Map objects” en un Complex, el breadcrumb quedaba plano. La causa era que los tres `buildMapPanelState()` generaban `ElectricalBoardResource::getUrl('view', ['record' => $board])` sin ningún `via_*`; ambos puntos de entrada reutilizan ese mismo URL.

- Complex ahora envía `via_complex`; Terrain, `via_terrain`; Structure, `via_structure` y conserva el `via_terrain` de la ruta actual cuando existe. Así, la vista del board puede reconstruir la jerarquía ya implementada en `FieldOpsBreadcrumbs::electricalBoardAncestors()`.
- Se añadieron asserts de regresión a `ComplexFilamentTest`, `TerrainFilamentTest` y `StructureFilamentTest` para fijar los URLs contextuales del panel de mapa.
- **Validación:** sintaxis PHP y `git diff --check` en verde; el usuario verificó el caso real en navegador y confirmó “ya funciona como se espera”. La regresión automatizada quedó bloqueada antes de ejecutar aserciones por un fallo del harness local de la base `testing` (`RefreshDatabase` no puede recrear la tabla `migrations`; Sail posteriormente reportó “Docker is not running” aunque `docker ps` mostraba los contenedores). **WAIVER aprobado explícitamente por el usuario con GO técnico:** cobertura alternativa = los asserts añadidos + QA manual del flujo exacto reportado. No mezclar la reparación del harness con este fix funcional.
...
**Contexto:** el usuario aclaró tras ver el resultado de la sesión anterior: "Electrical Boards debe tener también el sys de breadcrumb, ten en cuenta que el punto de entrada puede ser diferente, todas las páginas relacionadas con Electrical Boards deberían tenerlo" — no solo Create. A diferencia de los otros 4 recursos, Electrical Board sigue visible en el sidebar y nunca tuvo `getResourceBreadcrumbs()` en View/Edit.

- Renombrado `electricalBoardCreateAncestors()` → `electricalBoardAncestors()` (nunca fue realmente específico de Create) + nuevo `electricalBoardTrail()` (incluye la entrada propia del board) para el branch de ElectricalBoard en `maintenanceWorkOrderAncestors()`, que antes siempre mostraba la cadena plana.
- Nuevo query param `via_complex` (no existía antes) + `via_terrain`/`via_structure` propagados end-to-end: los 3 `ElectricalBoardsRelationManager` (Complex/Terrain/Structure) los mandan en `recordUrl()`, `ViewElectricalBoard`/`EditElectricalBoard` los leen, y `scheduleMaintenance` los reenvía hacia el work order.
- **Bug real preexistente encontrado de paso:** `Structures/RelationManagers/ElectricalBoardsRelationManager.php` construía `terrain_ids` con `->terrains()->pluck('id')` — columna ambigua entre `fo_terrains.id` y el join del pivot, tiraba SQL crudo apenas algo renderizaba los `headerActions` de verdad (nunca había pasado hasta este test). Corregido a `->pluck('fo_terrains.id')`.
- **Tests/checks:** +5 tests entre `FieldOpsHierarchyNavigationTest`/`ElectricalBoardFilamentTest`/`MaintenanceWorkOrderFilamentTest` — regresión completa 98/98 (452 assertions). Verificado en Chrome real navegando por click real desde la pestaña "Electrical boards" de una Structure.
- Detalle completo en `CLAUDE.md`, sección "FieldOps: breadcrumbs jerárquicos en páginas Create... y en TODAS las páginas de Electrical Board".

### Sesión 2026-08-01 (cont. 2) — FieldOps: breadcrumb en Luminaire Frame's Create page, olvidado en la primera pasada (commit `2dc5936`)

**Contexto:** el usuario señaló "olvidamos Luminaire Frames create de tener el mismo sistema se aplica" — quinto recurso con `shouldRegisterNavigation = false` que se quedó fuera del barrido inicial de Create pages.

- Nuevo `FieldOpsBreadcrumbs::luminaireFrameAncestorsForStructure()`, mismo patrón "solo con el padre" que los otros 4. `luminaireFrameAncestors()` refactorizado para delegar en él.
- **Tests/checks:** `FieldOpsHierarchyNavigationTest` +2 — regresión ampliada 62/62 (328 assertions). Verificado en Chrome real: `/luminaire-frames/create?structure_ids[0]=1` renderiza la cadena completa.
- Detalle en `CLAUDE.md`, misma sección que arriba.
...
- 4 métodos nuevos en `FieldOpsBreadcrumbs`: `terrainAncestorsForComplex()`, `structureAncestorsForTerrain()`, `luminaireAncestorsForStructure()`, `electricalBoardCreateAncestors()` (renombrado luego a `electricalBoardAncestors()`, ver sesión más reciente arriba).
...
**Contexto:** el usuario reportó, tras validar el trabajo de breadcrumbs, que navegar a Terrain/Structure/Luminaire recargaba la página completa, y que el breadcrumb no cubría la página de "Schedule maintenance".

- **Bug 1, no relacionado al breadcrumb:** varios partials Blade custom (`map-panel`, `profile-header`, `associated-complexes`, `used-by`, `luminaire-operational-overview`, y links sueltos en `luminaire-frame-spatial-layout`/`luminaire-type-gallery-selector`) construían `<a href="{{ $url }}">` en crudo para navegación interna, sin pasar por `\Filament\Support\generate_href_html()` — nunca llevaban `wire:navigate`. Confirmado empíricamente con Selenium (marcador en `window` seteado antes del click desaparecía después = contexto JS destruido = recarga real). `map-panel.blade.php` tenía además un segundo bug independiente: los pines de Leaflet navegaban con `window.location.href` en JS puro, no solo la lista lateral. `associated-complexes.blade.php` tenía un tercer patrón (`onclick="window.location=...'` en el `<tr>` completo). Los 8 archivos corregidos.
- **Bug 2:** `FoMaintenanceWorkOrderResource` nunca tuvo `getResourceBreadcrumbs()` — se agregó `FieldOpsBreadcrumbs::maintenanceWorkOrderAncestors()`, reutilizando la cadena completa de Luminaire (`luminaireTrail()`, nuevo) o anclando en el índice propio para ElectricalBoard (no forma parte de la jerarquía M:N oculta).
- **Tests/checks:** `MaintenanceWorkOrderFilamentTest` +3, `ComplexFilamentTest` +1 — regresión targeted completa 75/75 (423 assertions). Verificado end-to-end en Chrome real (Selenium): click real en "Schedule maintenance" mantiene SPA y renderiza el breadcrumb completo de 13 entries en una línea.
- Detalle completo en `CLAUDE.md`, sección "CLA-278 (cont. 10)".
...
**Contexto:** el usuario pidió tres cosas sobre `/luminaires/create`: (1) poder buscar un tipo de luminaire concreto o filtrar por subgrupo, porque el catálogo puede crecer; (2) subir foto/vídeo/documento asociado a la luminaria (no existía ningún soporte de media en el modelo); (3) UX progresiva — mostrar solo la grilla al inicio, ocultar la lista y mostrar la imagen seleccionada + el resto del formulario una vez elegido un tipo, siempre permitiendo cambiar la selección.

- Implementado: buscador + chips de subgrupo (Alpine, client-side) en `luminaire-type-gallery-selector.blade.php`; UX progresiva vía `Get $get`/`->visible()` en las Sections de `LuminaireResource::form()`; media (fotos/vídeos/documentos) en `Luminaire` replicando `HasFieldOpsMedia` (mismo patrón que Complex/Terrain/Structure/ElectricalBoard), con `videos` como colección nueva en el trait compartido.
- **Bug encontrado en QA manual con el usuario:** tras ocultar el resto del formulario hasta seleccionar un tipo, el usuario reportó que el submit "pasaba" sin seleccionar luminaria. Investigado con Playwright: la validación server-side (`->required()`, ya existente) **sí bloqueaba correctamente** el submit — el problema real era que el Blade custom del `ViewField` nunca renderizaba el error de Filament, así que el usuario no veía ningún feedback (parecía que no hacía nada). Corregido mostrando `$errors->first($statePath)` + borde rojo.
- **Bug preexistente encontrado de paso (no de este ticket, corregido con aprobación explícita):** `fo_luminaires.serial_number` es `NOT NULL` en BD pero el form Filament lo marcaba `->nullable()` — dejarlo vacío tiraba un 500 de SQL crudo. Se replicó en el backoffice la misma lógica que ya usaba la API/field-app (`LuminaireController::resolveSerialNumber()`, genera `AUTO-{timestamp}-{random}`), extraída a `LuminaireResource::resolveSerialNumber()` para no duplicarla en Create y Edit.
- **Validación:** 101/101 tests en el filtro `Luminaire|FieldOpsMedia` (incluye `FieldOpsMediaTest` completo). Suite `FieldOps` completa: 264 passed / 88 failed, los 88 son el harness preexistente `RoleAlreadyExists` (seeding de roles en tests paralelos), ninguno relacionado. QA manual en navegador real con Playwright (usuario de prueba temporal, creado y borrado en la misma sesión): búsqueda, filtro, ocultar/mostrar grilla, "Cambiar producto", error visible, submit exitoso, upload de foto persistido y visible en `/luminaires/{id}`.
- **Segunda ronda de feedback del usuario tras revisar el resultado, ya implementada:** (1) el producto instalado no puede cambiarse desde Edit — eso es exclusivo de "Replace luminaire" en la vista de detalle; se bloqueó con doble candado UI+servidor. (2) una luminaria no puede quedar huérfana: debe resolver Complejo→Terreno→Estructura→Frame — el `Select` plano de Frame en Create se reemplazó por una cascada de 4 selects (los 3 primeros son UI scaffolding, `dehydrated(false)`, requeridos solo en `create`). Bug real encontrado en el camino: `whereHas('terrains', ...)`/`whereHas('structures', ...)` usaban el nombre de la relación como prefijo de columna (`terrains.id`) en vez del nombre real de tabla (`fo_terrains.id`) — corregido. 101/101 tests tras ambos fixes.
- **Segunda ronda de QA manual del usuario en el navegador (2026-07-26), ya implementada:** (1) aviso reactivo de posiciones de frame ya ocupadas + fix de `frame_position` negativo/cero (500 crudo por `unsignedInteger`) + validación de posición duplicada (antes tiraba `UniqueConstraintViolationException` cruda). (2) Bug real, no específico de Luminaire: las galerías de fotos/video en las 5 resources FieldOps (Complex/Terrain/Structure/ElectricalBoard/Luminaire) usaban `$media->getUrl()` de Spatie directo, que 404 siempre para el disco privado `local` — fix con ruta nueva `fieldops.admin.media.show` (`auth` + `EnsurePanelAccess`). (3) Mismo patrón: la sección de documentos nunca tuvo un link real, solo un conteo — fix con partial `document-list.blade.php` en las 5 resources. (4) Hallazgo sin relación con CLA-278, encontrado de rebote: un test de `ComplexFilamentTest` afirmaba ver contenido de un relation manager lazy-loaded vía `$this->get()` plano, algo estructuralmente imposible — corregido a `Livewire::test()` directo sobre el relation manager.
- **Ticket Linear:** **CLA-278**, In Progress. Falta: marcar Done en Linear con el hash (o seguir sumando QA si el usuario encuentra algo más).
- Detalle completo de diseño en `CLAUDE.md`, secciones "CLA-278 — Create Luminaire: buscador de tipo, UX progresiva y media", "CLA-278 (cont.) — bloqueo del producto en Edit + jerarquía...", "CLA-278 (cont. 2) — QA manual del usuario: posición ocupada, media privada y documentos", "CLA-278 (cont. 3) — Frame Types: imágenes reales del seed + bug real de navegación" y "CLA-278 (cont. 4) — mismo bug de wire:navigate en el canvas espacial + fix de UX".

### Sesión 2026-07-27 (cont.) — CLA-278: mismo bug de wire:navigate en el canvas de Luminaire Frame + fix de UX (commit `9f2ef37`)

- El usuario reportó, probando el flujo de agregar una luminaria, que el "Frame overview" de `/luminaire-frames/{id}` cargaba completamente vacío al llegar por click desde el listado. Misma causa raíz que la sesión anterior: `luminaire-frame-spatial-layout.blade.php` registraba su componente Alpine (`window.fieldopsLuminaireFrameLayout = function(...)`) dentro de `@push('scripts')`/`@once` — inerte bajo `wire:navigate`. Mismo fix: `@script`/`@endscript` + `Alpine.data(...)`.
- **Bug de UX real encontrado por el usuario en la misma sesión, distinto del bug de JS:** el canvas (imagen del frame + grid) solo se mostraba con al menos 1 luminaria ya colocada — un frame nuevo sin luminarias mostraba solo texto genérico, sin la imagen del frame para poder ubicar la primera luminaria visualmente. Fix de una línea: mostrar el canvas también cuando el frame type tiene imagen, aunque `count($markers) === 0`.
- **2/2 archivos auditados en vivo (con Selenium) tenían el mismo anti-patrón de `wire:navigate`** — sube la prioridad de auditar los 4 `*-location-picker.blade.php` que quedaron documentados como riesgo sin arreglar en `docs/ai/known-risks.md` (sección "Alpine.data() + alpine:init muerto en 4 location-pickers bajo wire:navigate").
- 20/20 tests en verde (`LuminaireFrameFilamentTest`, `LuminaireFrameCrudTest`). Verificado end-to-end con Chrome real (Selenium), navegación por click real. Detalle completo en `CLAUDE.md`, sección "CLA-278 (cont. 4)".

### Sesión 2026-07-26/27 — CLA-278: Frame Types — imágenes reales del seed + bug real de wire:navigate (commit `a935834`)

- El usuario pidió reemplazar los placeholders del catálogo `LuminaireFrameType` (`Traverse 1-5`, `Balcony`, sin imagen) por 6 headframes reales con sus imágenes (`public/assets/frame-types/`, mismo patrón que `LuminaireType`). Colateral necesario: fix de un FK hardcodeado en `FieldOpsDemoDataSeeder` y fix de `ImageColumn` no resolviendo rutas fuera del disco `public` en `LuminaireFrameTypeResource` (mismo bug que ya se había arreglado antes en `LuminaireTypeResource`).
- **Bug real reportado por el usuario, no relacionado con las imágenes:** el editor de imagen de Frame Types (Edit) no mostraba nada ni respondía a los botones — pero solo al llegar por click desde el listado, no cargando la URL directo. Causa raíz real: el `<script>` que registraba el componente Alpine vivía en `@push('scripts')`/`@once`, que nunca se ejecuta cuando Filament navega por `wire:navigate` (DOM morph, no recarga completa — los navegadores no corren `<script>` insertados así). Migrado a `@script`/`@endscript` (Livewire) + quitado un `addEventListener('alpine:init', ...)` que ya no tenía sentido con `@script` (ese evento solo dispara una vez por sesión de navegador). Diagnosticado y verificado con Chrome real vía Selenium (WebDriver REST crudo desde el contenedor, usuario QA temporal borrado al cierre) — los tests de HTML estático nunca hubieran detectado esto.
- **Hallazgo de alcance, sin corregir a pedido del usuario:** el mismo patrón roto existe en `complex-location-picker`, `terrain-location-picker`, `structure-location-picker` y `electrical-board-location-picker`. `luminaire-type-gallery-selector` (picker de Create Luminaire) se revisó y confirmó que NO tiene el bug (usa `x-data` inline). Pendiente: ticket propio para los 4 location-pickers.
- 18/18 tests en verde. Detalle completo en `CLAUDE.md`, sección "CLA-278 (cont. 3)".

### Sesión 2026-07-24 — CLA-276: alertas operacionales + métricas de ciclo de vida (backend `26110b5`)

**Contexto:** último ítem del checklist de Fase 5 que se puede implementar en código (a diferencia de la infraestructura de producción, esto sí es una feature normal). Investigué los patrones existentes (alertas de Mailing, Watchdog de Performance, compliance de Safety) antes de diseñar, para no inventar un patrón nuevo — el de Mailing (comando + tabla de alertas con dedupe + `Notification` config-driven) resultó ser el precedente más transferible. El usuario decidió el alcance vía 3 preguntas: ambas alertas (sin primera respuesta + sin confirmar), sí construir el widget de dashboard, runbook en `production-readiness.md`.

- **Alertas:** `fo_maintenance_request_alerts` (una fila por `(request, alert_type)`, reutilizada entre ciclos — no acumula historial, y sí puede re-alertar tras una reapertura). `MaintenanceRequestAlertType`: `no_first_response` (received/in_review, `acknowledged_at` nulo, más viejo que `FIELDOPS_REQUEST_ALERT_FIRST_RESPONSE_HOURS`=24h) y `awaiting_confirmation` (resolved, `confirmed_at` nulo, más viejo que `FIELDOPS_REQUEST_ALERT_CONFIRMATION_WAIT_DAYS`=7d). `MaintenanceRequestAlertService` dispara vía `updateOrCreate` (idempotente), auto-resuelve cuando la condición se limpia, notifica a `admin`/`super_admin` (misma query que `MaintenanceRequestService::notifyBackoffice`). Comando `fieldops:check-request-alerts` (`--dry-run` soportado), programado hourly junto a `fieldops:generate-maintenance-work-orders`.
- **Dashboard:** `MaintenanceRequestLifecycleWidget` (StatsOverviewWidget en la cabecera de Service Requests) — solicitudes recibidas, tiempo promedio a primera respuesta/asignación/resolución/confirmación, tasa de reapertura, alertas abiertas. Calculado en vivo sobre ventana móvil de 30 días, sin tabla de rollup diario (volumen no lo justifica todavía, a diferencia de Safety).
- **Runbook:** nueva sección en `docs/ai/production-readiness.md` (qué hacer con cada alerta) y entrada nueva en `docs/ai/commands-runbook.md`.
- **Gotcha real encontrado en la migración:** el nombre de índice único autogenerado por Laravel (`fo_maintenance_request_alerts_maintenance_request_id_alert_type_unique`) supera el límite de 64 caracteres de identificador de MySQL — la tabla se creaba pero el `ALTER TABLE ADD UNIQUE` fallaba en silencio dejando la tabla huérfana (sin registrar en `migrations`), lo que al principio pareció un problema de concurrencia entre sesiones hasta que se aisló la causa real. Corregido nombrando el índice explícitamente.
- **Tests:** 9 casos nuevos (dispara/no dispara, idempotente, auto-resuelve, re-dispara tras reapertura, dry-run es no-op real, widget + list page renderizan) — **9/9, 28 aserciones**. Regresión focal (MaintenanceRequestTest, ClientPortalInfrastructureTest, StructureType/StructureFilamentTest) — **39/39, 209 aserciones**. Pint limpio. Migración validada up→rollback→up.
- **Estado de Fase 5:** con esto, todo el checklist de Fase 5 tiene código implementado y verificado. Solo falta lo que requiere acceso real a infraestructura (aplicar el runbook de `client.claesen-verlichting.be` en `sbapu03`/`prod-priv-01`) y decisiones de negocio pendientes (pipeline de CI/CD, métricas de adopción de producto vía Analytics). No cerrar CLA-276 en Linear hasta resolver eso.
...
- **Alcance:** separar capacidades create/update/delete/delete-infrastructure de `fieldops.view-all-clients` (que hasta ahora también funcionaba como bypass de escritura). Create: 5 recursos (Complex sin ruta POST). Update/delete: 6 recursos, Complex incluido. `fieldops.media`/`fieldops.ai` creados como fundación para CLA-498/CLA-502, deliberadamente inertes (sin ningún controller verificándolos todavía).
- **Split de `FieldOpsInfrastructurePolicy`** (view/create/update/delete, 6 recursos de infraestructura) separada de `FieldOpsTenantPolicy` (sin cambios, `FoClient`+3 modelos de mantenimiento) — necesario porque Laravel descarta el class-string antes de invocar una ability `create($user)`, así que una policy compartida por los 10 modelos de `TENANT_MODELS` no podía diferenciar qué modelo disparó la llamada.
- **Middleware:** quitado el early-return de `hasBroadAccess()` (redundante para `view`, pero ocultaba el gap de escritura); agregado dispatch `update`/`delete` en PUT/PATCH/DELETE solo para el allowlist de infraestructura — todo lo demás (POST, mantenimiento) conserva `view` exactamente igual que antes.
- **Backfill fail-safe:** migración nueva (no seeder — `deploy.sh` nunca corre uno) que crea los 5 permisos siempre y concede solo si el rol ya existe (`Role::where(...)->first()`, nunca `findByName()`), segura en `migrate:fresh` con tabla de roles vacía. `down()` simétrico.
- **Resultado focal + gate serial (mismo filtro histórico ampliado con 2 clases nuevas, una sola invocación):** 216 passed / 677 assertions, 635.31s — base histórica era 180/520 sobre 8 clases, el crecimiento corresponde exactamente a las clases/tests añadidos.
- **QA real** contra el servidor de dev (no Selenium, este ticket no toca frontend): `project_manager`/`technician` reales con tokens Sanctum reales vía `curl` — confirmado `project_manager` crea (201) pero no borra (403), `technician` actualiza su propio Complex (200) pero no lo borra (403). El caso cross-tenant (`technician` bloqueado al actualizar equipo de otro cliente) se verificó vía PHPUnit (`test_technician_can_update_within_scope_but_not_cross_tenant_and_never_delete`), no vía `curl`. `migrate`/`rollback`/`migrate` real contra la DB de dev, no solo en aislamiento de test. Datos de QA borrados al terminar.
...
**Reapertura, mismo día — gap de baseline en producción:** auditoría post-cierre (`permission:show` contra un entorno real) encontró que `fieldops.view-all-clients` (CLA-364) y los roles `technician`/`client` solo se crean en `RolesAndPermissionsSeeder`, nunca en una migración — `deploy.sh` nunca corre un seeder. Un entorno cuya última corrida del seeder es anterior a CLA-364/esos roles queda sin ese baseline: sin la fila `fieldops.view-all-clients`, `hasBroadAccess()` da `false` siempre y `super_admin`/`admin` pierden todo acceso amplio a FieldOps. Fix: migración nueva `2026_08_29_037_backfill_fieldops_baseline_roles_and_permissions.php` (no toca la 036), `down()` deliberadamente no destructivo. Detalle completo, gotcha de tests (RefreshDatabase corre la migración real antes de cada test, rompiendo la premisa de 2 tests nuevos y 1 test ya comiteado de la 036) y validación (8 tests nuevos, gate serial de 11 clases 224 passed / 718 assertions sin fallos, `migrate`/`rollback`/`migrate` real contra dev) en `CLAUDE.md` → FieldOps → CLA-496 → adenda.

**Cierre final:** comiteado por separado (`f4374920a7103604450b0bc3855d48111b1c9e74`, sin tocar ni reescribir `b81e8a3`), publicado en `origin/main` (preflight confirmó `b81e8a3` como HEAD remoto antes del push) y verificado end-to-end en producción real por el usuario: migración 037 aplicada, matriz de permisos exacta, `technician` correcto (`create`/`update`/`media`/`ai`, nunca `delete-infrastructure`), `client` deliberadamente ausente, deploy real exitoso, único fallo de CI el falso negativo ya documentado (health-check contra el hostname privado `backoffice.claesen.local`). **CLA-496 cerrado en Linear (Done) el 2026-08-29** con esa evidencia publicada en el ticket.

### Sesión 2026-08-30 — CLA-497: tenant-scope en endpoints globales de mantenimiento

**Contexto:** segundo hallazgo de la misma auditoría de seguridad FieldOps que originó CLA-496. `MaintenanceRecordController::correctiveStats()`/`pendingClientReported()`/`clientReportedStatistics()` no tienen parámetro de ruta ligado a un modelo, así que `EnforceFieldOpsTenantAccess` nunca los autoriza — cualquier actor no-`client` (incluido `technician`, el único rol interno scoped) recibía agregados y PII de todos los clientes. Detalle completo (queries antes/después, matriz de actores, PII expuesta) en `CLAUDE.md` → FieldOps → CLA-497.

- **Fix:** `scopeForUser()` (ya existente en `FieldOpsTenantService`, sin tocarlo) aplicado en la query de los 3 métodos, con `Request $request` inyectado (nunca `request()` global) — decisión: scoped, no 403; `hasBroadAccess()` sigue viendo todo (CLA-377 intacto); `client` sigue bloqueado por el middleware, sin cambios.
- **Regresión corregida:** `ClientReportedMaintenanceTest::token()` dependía del propio bug (usuario sin permiso viendo todo) — mismo tratamiento CLA-378/369.
- **Tests nuevos:** `MaintenanceRecordTenantScopeTest.php` (8 tests) — 2 `FoClient` reales con `client_id` explícito, PII única por cliente con `assertDontSee` cruzado, shape exacto de respuesta vacía (`assertExactJson`) + 401 en las 3 rutas. **Cobertura del actor amplio corregida tras revisión** (la versión inicial solo comprobaba un conteo): `project_manager` ahora tiene un test con fixtures 100% deterministas (ancla `Carbon` única, offsets fijos) que afirma `assertExactJson` sobre la respuesta agregada completa de `correctiveStats()` y `clientReportedStatistics()` (A+B combinados) y confirma que `pendingClientReported()` trae ambos registros pendientes con su PII propia intacta; `admin` queda como regresión representativa más liviana (conteo), a propósito.
