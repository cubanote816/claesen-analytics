...
> Última actualización: 2026-07-07 — membrete corporativo oficial agregado a todos los PDFs (`core::pdf.letterhead`), rebranding completo "Claesen Intelligence Hub" → "Claesen Outdoor Lighting Platform", y bug real encontrado en `MicrosoftGraphTransport`: los logos embebidos por CID en emails salían como adjunto suelto (no inline) porque Graph API necesita `isInline`/`contentId` explícitos — sin este fix, **ningún** logo embebido en email llegaba a mostrarse inline en producción, incluido el fix de la sesión anterior. Detalle completo en memoria `project_official_branding`. El resto sigue igual: segundo incidente de producción (`SESSION_DOMAIN`/Sanctum) resuelto; fix logo roto en email/PDF de inspecciones Safety, commiteado y pusheado (`c63c8d4`); ElectricalBoard, Media (FileLibrary) y Client encontrados igual de rotos que Terrain/Structure/Luminaire; los 3 arreglados y verificados en vivo, sigue sin commitear.
...
- **Pendiente:** confirmar visualmente en un correo real recibido (no solo render de prueba) que el logo ahora sale inline vía Microsoft Graph con el fix de `isInline`/`contentId` — el render de Laravel no puede probar esto end-to-end porque no pasa por el transport real.
