...
> Última actualización: 2026-07-06 — incidente de producción resuelto: login de Azure OAuth roto tras deploy (`SESSION_DOMAIN` desincronizado + gap de opcache), fix aplicado en caliente y `deploy.sh`/docs actualizados para que no se repita. El resto sigue igual: fix logo roto en email/PDF de inspecciones Safety, commiteado y pusheado (`c63c8d4`); ElectricalBoard, Media (FileLibrary) y Client encontrados igual de rotos que Terrain/Structure/Luminaire; los 3 arreglados y verificados en vivo, sigue sin commitear.
