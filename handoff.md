...
> Última actualización: 2026-08-03 — **CLA-294** integró la `main` local en `fieldops-backend-fixes` antes de preparar el merge final. Los conflictos de Core conservaron a la vez las redirecciones OAuth seguras, las relaciones/preferencias de FieldOps y la analítica de accesos. Validación focal Core/FieldOps ejecutada vía Sail, además de `php -l` y `git diff --check` limpios. Pendiente operativo: subir la rama y abrir/revisar el PR hacia `main`.
