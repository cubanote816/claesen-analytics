...
| CLA-276 | Fase 5 — Validación integral y producción de Claesen-Client | 🚧 En progreso — E2E de `luminaire_id` (backend `3680602`), refactor de Claesen-Client en componentes testeables + 52 tests (frontend `79b11f7`/`c60e06b`), cancelación de solicitudes cliente/backoffice pre-conversión (backend `cb33822`, frontend `3e9893e`), auditoría y fixes WCAG 2.2 AA (frontend `5fde0c0`), artefactos + runbook de infraestructura de producción para `client.claesen-verlichting.be` (backend `03660ae`, sin ejecutar — requiere acceso SSH/DNS real). Pendiente: aplicar el runbook, decidir pipeline de CI/CD y monitorización/métricas |
