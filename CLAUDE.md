...
- **Fix visual inmediato reportado por el usuario (commit `f4bbfd4`):** con jerarquías profundas (10+ segmentos) el breadcrumb de Filament (`flex-wrap` nativo) se rompía en 2-3 líneas con el separador "›" quedando colgado al inicio de cada línea. Fix panel-wide (no específico de FieldOps) en `theme.css`: `.fi-breadcrumbs-list` pasa a una sola línea con scroll horizontal (`flex-wrap: nowrap` + `overflow-x: auto`). Nuevo `core::filament.breadcrumbs-scroll-to-current`, registrado en `BODY_END` junto a `sidebar-scroll-restore` (mismo patrón ya establecido): escuchas globales de `livewire:navigated` que scrollean el breadcrumb a su extremo derecho (el segmento actual) en cada carga/navegación — no un `Alpine.data`/`@script` por componente, así que no aplica el gotcha de `wire:navigate` de `feedback_livewire_wire_navigate_alpine` (los listeners se registran una sola vez al cargar la página real y quedan escuchando el evento global de Livewire para siempre). Requiere `npm run build` tras tocar `theme.css` (Vite), ya corrido. Verificado en Chrome real contra la página exacta del reporte.
