...
| CLA-386 | Identificación visual de luminarias en frame — MVP de backend con Claude Sonnet 5 | ✅ Done — ver detalle abajo. **Solo backend**, sin consumidor en Claesen-Sport todavía → follow-up CLA-388 |
| CLA-388 | Conectar identificación visual (foto + confirmación) en Claesen-Sport | ⬜ Todo — ver detalle en CLA-386 |
...
- **Gap real confirmado por auditoría de código, no de memoria (2026-08-18):** este ticket es **solo backend**. Grep exhaustivo sobre `/home/totti/Claesen-Sport` no encontró ningún código de `vision`/`identify`/`suggestion`/captura de foto ligado a luminarias — el endpoint no tiene ningún consumidor todavía. El selector manual de catálogo que sí existe hoy (`LuminaireFormModal.tsx`) son dos `<select>` HTML en cascada (Subgrupo → Tipo) sin buscador ni galería, alimentados por el mismo catálogo estático interno de siempre (sin cambios desde el commit `d8fdade`, 2026-07-05) — **no hay ninguna consulta en vivo a catálogos de fabricantes** (Philips/Schréder/Thorn/etc.), pese a que el flujo conceptual descrito por el usuario asumía que sí. Follow-up creado como **CLA-388** (Todo, sin iniciar) para conectar el flujo foto→sugerencia→confirmación en Claesen-Sport.
