...
| CLA-578 | FieldOps: Reporting & scheduling API (F6a del rediseño UI/UX) — filtros + paginación en `/maintenance-work-orders/history`, rango de fechas en `/assigned`, `/stats`, `/export` (CSV) e índice `GET /luminaires` tenant-scoped | 🚧 In Progress — rama `cla-578-fieldops-reporting-api`, 13 tests nuevos en verde, pendiente GO técnico. Sub-issue CLA-579 (checklist de werkorder) sin empezar |
