...
- Fix del bug `[object Object]` en el Edit de Terrain Types: mismo patrón que `EditTerrain.php` (`mutateFormDataBeforeFill()` resolviendo el locale actual), porque `EditRecord::fillForm()` usa `attributesToArray()`, que bypassa el accessor de traducción de Spatie. **Corregido también en AccessType, StructureType, SafetyType y ElectricalBoardType (CLA-273, mismo día)** — `LuminaireFrameType`/`LuminaireSubgroup`/`LuminaireType` quedaron fuera a propósito porque no usan `HasTranslations` (columnas string planas), nunca tuvieron este bug.
