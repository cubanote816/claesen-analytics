...
| FO-006 | Slice C.6b — Cutover: frontend Sport → Core, deprecar Sport | 🚧 En progreso — decisión de deprecar Sport confirmada (CLA-328, 2026-09-21). Exploración técnica (2026-09-22) encontró 3 capacidades sin equivalente en Core: adjuntar fotos (trivial, `FileUpload` estándar), flujo de ejecución (buildable vía `Wizard`, ver **CLA-581**), **offline PWA — pared arquitectónica confirmada, Livewire pausa toda interacción sin señal y Filament no trae infraestructura PWA; queda explícitamente fuera de alcance, sigue viviendo en una app cliente separada**. CLA-581 migró el flujo de ejecución (rol `technician` con acceso al panel restringido a `Modules/FieldOps/Filament/Pages/MyWorkOrders`) — si el cutover necesita mantenimiento *programado* a futuro, abrir ticket nuevo para `ScheduledMaintenanceService` antes de cerrar C.6b |
