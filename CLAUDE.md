...
| CLA-389 | Identificación visual contra el catálogo completo de marcas (Philips/Signify, Schréder, Thorn, Musco), no solo los 10 tipos internos | ⬜ Todo — ver detalle abajo |
...
### CLA-389 — identificación visual contra el catálogo completo de marcas (2026-08-19, Todo, sin iniciar)

Follow-up de CLA-388. Dos diseños evaluados con el usuario: (A) ampliar `fo_luminaire_types` a mano con modelos reales de Philips/Signify, Schréder, Thorn y Musco — descartado, es exactamente lo que CLA-386 original marcó como fuera de alcance ("requeriría cola de revisión humana antes de publicar cualquier dato"), mucho trabajo de curaduría antes de dar valor; (B) **elegida** — cuando `ClaudeVisionService` no matchea contra el catálogo interno, que devuelva además una sugerencia de marca+modelo en texto libre basada en el conocimiento propio del modelo sobre esas 4 marcas, explícitamente no autoritativa, con confirmación humana antes de crear la entrada nueva en el catálogo (mismo patrón que el intake de IA no autoritativo de CLA-268). Detalle completo del diseño pendiente de definir en el plan del ticket — ver Linear CLA-389.
