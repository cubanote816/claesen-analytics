...
class ExecuteCampaignJobCounterTest extends TestCase
{
    ...
    public function test_unsubscribed_prospect_creates_unsubscribed_status_message(): void
    {
        $this->mockMailer(true);
        $region   = $this->region();
        $p1       = $this->prospect($region);
        $p1->update(['unsubscribed_at' => now()]);
        $campaign = $this->campaign();
    ...
    public function test_suppressed_unsubscribed_prospect_creates_unsubscribed_status_message(): void
    {
        $this->mockMailer(true);
        $region   = $this->region();
        $p1       = $this->prospect($region);
        $email    = $p1->locations()->first()->email;

        // Suppress using SuppressionService
        app(SuppressionService::class)->suppress($email, \Modules\Mailing\Enums\SuppressionReason::UNSUBSCRIBED);
    ...
    public function test_suppressed_other_prospect_creates_skipped_status_message(): void
    {
        $this->mockMailer(true);
        $region   = $this->region();
        $p1       = $this->prospect($region);
        ...
        // Suppress using SuppressionService (hard bounce reason)
    ...
    public function test_prospect_without_email_creates_skipped_status_message(): void
    {
        $this->mockMailer(true);
        $region   = $this->region();
        // Create a prospect and delete their locations to simulate no email
        $p1       = $this->prospect($region);
        $p1->locations()->delete();
