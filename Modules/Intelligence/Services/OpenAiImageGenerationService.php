...
class OpenAiImageGenerationService
{
    ...
    {
        return <<<'PROMPT'
You are generating a technical catalog illustration for a sports-lighting contractor's internal parts catalog. The last attached image is a real photo taken by a field technician of a physical lighting frame structure at a job site. The preceding attached images are the exact house visual style already used for every other frame-type catalog entry: isolated structure only, no luminaires mounted, no background scenery, flat even lighting, slightly technical/illustrative rendering (not a real photograph).

The technician's photo may show luminaires, cameras, cables, or other equipment mounted on the frame, plus background scenery or clutter. IGNORE ALL OF THAT COMPLETELY when judging what to draw — you are illustrating only the bare structural frame itself, not anything attached to it. Generate a NEW image of ONLY that bare frame structure, redrawn from scratch in the exact catalog house style (same camera angle/framing conventions, same grey/silver structure with subtle green accents where applicable). Reproduce the EXACT structural shape visible in the technician's photo — same mast, same arms/cross-bars, same number and angle of tiers — do not add mounting brackets, plates, gussets, or any other structural hardware that is not clearly visible in the technician's photo, even if the reference images show such hardware; the reference images are a STYLE guide (rendering, lighting, material, line weight) only, never a shape template — never merge or borrow structural features from them. Where the photo shows a bare pipe/bar end with nothing attached, draw it as a bare pipe/bar end. Do NOT draw any luminaire, lamp head, camera, sensor, cable, or other mounted equipment anywhere in the result, even if the technician's photo has one mounted. Do not literally edit or crop the technician's photo — draw a clean illustration of only the structure. The background must be fully transparent.
PROMPT;
    }
